...
async function coletarPromocoesFinal8(force, digito = 8) {
  ...
  try {
    ...
    // ATUALIZADO (Tiago, 18/09): já estava em promoção e o preço mudou continuando com final 8 → marca por 10 dias, com o preço anterior
    for (const r of rows) for (const ln of r.promoLojas) { const k = r.cod + '|' + ln; atuais.add(k); const pr = r.lojas[ln].preco;
      const v = vistos.itens[k];
      if (!v) { vistos.itens[k] = primeira ? { desde: hojeStr, base: true, preco: pr } : { desde: hojeStr, preco: pr }; mudou = true; }
      else if (v.preco == null) { v.preco = pr; mudou = true; }                                   // registro antigo, sem preço guardado: só aprende
      else if (Math.abs(v.preco - pr) >= 0.005) { v.precoAnterior = v.preco; v.preco = pr; v.alteradoEm = hojeStr; mudou = true; } }
    ...
    for (const r of rows) { let novoDesde = null;
      for (const ln of r.promoLojas) { const v = vistos.itens[r.cod + '|' + ln]; const l = r.lojas[ln]; l.desde = v ? v.desde : null;
        ...
        l.atualizado = !!(v && v.alteradoEm && !l.novo && dias(v.alteradoEm) < PROMO_NOVO_DIAS);
        l.atualizadoEm = l.atualizado ? v.alteradoEm : null; l.precoAnterior = l.atualizado ? v.precoAnterior : null; }
      r.novo = !!novoDesde; r.novoDesde = novoDesde; r.novoLojas = r.promoLojas.filter(ln => r.lojas[ln].novo);
      r.atualizadoLojas = r.promoLojas.filter(ln => r.lojas[ln].atualizado);
      r.atualizado = !r.novo && r.atualizadoLojas.length > 0;
      r.atualizadoEm = r.atualizado ? r.atualizadoLojas.map(ln => r.lojas[ln].atualizadoEm).sort().pop() : null; }
...
app.get('/api/promocoes/final-8.csv', async (req, res) => {
  try {
    ...
    head.push('Lojas em promoção', 'Estoque em promoção', 'R$ custo em promoção', 'Novo desde', 'Lojas novas', 'Atualizado em', 'Lojas atualizadas');
...
// Comprador(a) aceita e fecha a consistência de uma loja (motivo obrigatório)
...
async function gerarPedidoErpTeste(p, usuario, lojasPedidas) {
  ...
  if (!lojas.length) return resultado;
  ...
  for (const ln of lojas) {
    ...
    else resultado.erros.push({ loja: ln, erro: r.erro, logId: r.id, em: new Date().toISOString() });
  }
...
// Cotações de TESTE (Tiago, 15/09: "atalho em cada tela pra eu analisar tela a tela" + "deixa vários exemplos na tela"):
// cria 5 cotações de exemplo em situações diferentes — pedido gerado (com pedidos em Pedidos de Compra), pronta pra
// análise, em digitação, não digitada e cancelada — com itens reais da lista 277 (se o Radar já calculou; senão amostra)
// e fornecedores de teste. Nada vai pro ERP nem pra vendedor real. "Remover testes" apaga tudo.
async function criarCotacaoTeste(usuario, cenario, diasAtras, seq) {
  const det = radarPedidos.itensLista(277, radarPedidos.TETO_PADRAO, 0, undefined, false, { alvo: 28, ponto: 3 });
  const base = det ? det.itens.filter(i => i.qtd > 0 && i.custo > 0) : [];
  const ini = (seq * 9) % Math.max(1, base.length - 12);
  let itens = base.slice(ini, ini + 8 + seq * 2).map(i => ({ cod: i.cod, descricao: i.descricao, unid: i.unid, emb: i.emb, lojas_qtd: i.lojas_qtd, ultimo_custo: i.custo, estoque: i.estoque, venda_dia: i.venda_dia, cobertura_dias: i.cobertura_dias, curva_a: i.curva_a }));
  ...
  if (cenario === 'vendedores') {
    // empresa com 3 vendedores → vira 3 concorrentes "DIA · Fulano" (cada um com link e preços próprios) + 1 concorrente comum
    forns.length = 0;
    forns.push({ codFornec: 999005, nome: 'DIA DISTRIBUIDORA (TESTE)', vendedor: { nome: 'Ana Teste', whats: '5581999990005', email: 'ana@dia.teste' }, vendedores: [{ nome: 'Bruno Teste', whats: '5581999990006', email: '' }, { nome: 'Clara Teste', whats: '5581999990007', email: 'clara@dia.teste' }], faturamento_minimo: 500, condicao: 'Boleto 28 dias', prazo_entrega: 3 },
               { codFornec: 999001, nome: 'FORNECEDOR TESTE 1 (SABOR & CIA)', vendedor: { nome: 'Maria Teste', whats: '5581999990001', email: 'teste1@exemplo.com' }, faturamento_minimo: 600, condicao: '28 dias', prazo_entrega: 3 });
  }
...
app.post('/api/precificacao/criar-teste', (req, res) => { const n = Math.min(5, Math.max(1, parseInt(req.body?.n) || 5)); const regs = precificacao.criarTestes(precifUser(req), n); res.json({ criados: regs.map(r => r.id), id: regs[0]?.id }); });
