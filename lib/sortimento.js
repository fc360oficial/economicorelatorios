...
async function calcular() {
  ...
  calculando = (async () => {
    ...
    // data exata da última venda (o histórico mensal só tem o mês): cupons do mês atual e do anterior, por loja
    const ultDia = {};
    try {
      const hoje = new Date(), meses2 = [new Date(hoje.getFullYear(), hoje.getMonth(), 1), new Date(hoje.getFullYear(), hoje.getMonth() - 1, 1)];
      const dIni = meses2[1].toISOString().slice(0, 10);
      for (const ln of LOJAS) for (const m of meses2) { const tb = `ln${ln}mes${String(m.getMonth() + 1).padStart(2, '0')}`;
        for (const ch of chunk(cods, 4000)) { const ph = ch.map(() => '?').join(',');
          for (const r of await q(`SELECT Codigo cod, DATE_FORMAT(MAX(Data),'%Y-%m-%d') d FROM \`${tb}\`.zcupomitens WHERE Data >= ? AND IndCancel='N' AND Codigo IN (${ph}) GROUP BY Codigo`, [dIni, ...ch]).catch(() => [])) { const k = `${r.cod}|${ln}`; if (!ultDia[k] || r.d > ultDia[k]) ultDia[k] = r.d; } } }
