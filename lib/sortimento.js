...
const NUM_COLS = { v6: 'v6', v12: 'v12', meses12: 'meses12', est: 'est', valorEst: 'valorEst', cob: 'cob', ent6: 'ent6' };
...
function csv(rows) {
  ...
  const head = 'Nº lista;Lista;Fornecedor;Comprador;Loja;Código;Descrição;Embalagem;Venda 24 m (un);Venda 24 m (R$);Venda 12 m (un);Venda 6 m (un);Meses com venda (12 m);Última venda;Estoque;Custo unit.;Valor estoque;Entradas 24 m;Entradas 6 m;Última entrada;Cobertura (dias);Classe';
  return '﻿' + [head].concat(rows.map(r => [r.lista, r.nome, r.forn, r.comprador || '', 'L' + r.loja + ' ' + NOMES[r.loja], r.cod, r.descricao, r.emb, r.v24, r.r24, r.v12, r.v6, r.meses12, r.ultVenda || '', r.est, r.custo, r.valorEst, r.ent24, r.ent6, r.ultEnt || '', r.cob, CLASSE_PT[r.classe] || r.classe].map(esc).join(';'))).join('\r\n');
}
