...
function csv(rows) {
  ...
  return '﻿' + [head].concat(rows.map(r => [r.lista, r.nome, r.forn, r.comprador || '', 'L' + r.loja + ' ' + NOMES[r.loja], r.cod, r.descricao, r.emb, r.v24, r.r24, r.v12, r.v6, r.meses12, r.ultVenda || '', r.ultVendaDia || '', r.est, r.custo, r.preco, r.margemCad ?? '', r.margemApl ?? '', r.ultCompra || '', r.ultCompraPreco ?? '', r.valorEst, r.ent24, r.ent6, r.ultEnt || '', r.cob, CLASSE_PT[r.classe] || r.classe].map(esc).join(';'))).join('\r\n');
}
