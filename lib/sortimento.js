...
async function calcular() {
  ...
  calculando = (async () => {
    ...
    for (const it of itens) for (const ln of LOJAS) {
      ...
      else if (v24 === 0 && en.q === 0) classe = consumo ? 'consumo_sem_movimento' : 'marcado_sem_movimento';
      ...
      rows.push({ grupo: it.grupo || null, subgrupo: it.subgrupo || null, consumo, lista: it.lista, nome: it.nome, forn: it.forn, comprador: compradorPorLista[it.lista] || null, loja: ln, cod: it.cod, descricao: it.descricao, emb: num(it.emb) || 1,
...
function filtrar({ loja, comprador, classe, lista, busca, faixas, ultDe, ultAte, consumo }) {
  if (!cache) return [];
