...
let deps = null, cache = null, calculando = null;
function init(d) { deps = d; try { cache = JSON.parse(fs.readFileSync(OUT, 'utf8')); } catch (e) { cache = null; } }
...
function agendar() {
  ...
  setTimeout(() => { if (!cache || cache.calculadoEm.slice(0, 10) !== hojeStr()) calcular().catch(e => console.error('[SORTIMENTO]', e.message)); }, 4 * 60 * 1000);
  setInterval(() => { const d = new Date(); if (d.getHours() === 5 && d.getMinutes() === 45) calcular().catch(e => console.error('[SORTIMENTO]', e.message)); }, 60 * 1000);
}
module.exports = { init, agendar, calcular, filtrar, resumo, csv, estado, NOMES };
