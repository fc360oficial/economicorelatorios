...
async function calcular() {
  ...
  calculando = (async () => {
    ...
    for (const ch of chunk(cods, 2000)) { const ph = ch.map(() => '?').join(',');
      ...
      for (const r of rows2) { const k = `${r.cod}|${r.nLoja}`; if (ultCompra[k]) continue; const qun = num(r.qun), vt = num(r.vt), vu = pp(r.vu); ultCompra[k] = { dt: r.dt, preco: qun > 0 && vt > 0 ? +(vt / qun).toFixed(4) : (vu > 0 ? vu : null) }; } }
    const rows = [];
