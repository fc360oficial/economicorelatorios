...
const isoDate = v => { if (!v) return null; if (v instanceof Date) return isNaN(v) ? null : `${v.getFullYear()}-${String(v.getMonth() + 1).padStart(2, '0')}-${String(v.getDate()).padStart(2, '0')}`; const s = String(v).trim(); let m = s.match(/^(\d{4})-(\d{2})-(\d{2})/); if (m) return m[1] + '-' + m[2] + '-' + m[3]; m = s.match(/^(\d{2})\/(\d{2})\/(\d{4})/); if (m) return m[3] + '-' + m[2] + '-' + m[1]; return null; };
...
// item de consumo (uso interno): grupo/subgrupo do mercadológico, nome da lista ou descrição
const CONSUMO_RE = /CONSUMO|USO INTERNO|USO E CONSUMO|MATERIAL DE (LIMPEZA|ESCRIT[OÓ]RIO|EXPEDIENTE|USO)|EMBALAGE|SACOLA|DESCART[AÁ]VE|UNIFORME|EPI\b|MANUTEN[CÇ][AÃ]O|ROTULO|ETIQUETA|BOBINA/i;
let idxAll = null;
...
async function calcular() {
  ...
  calculando = (async () => {
    ...
    const itens = await q(`SELECT i.nCotacao lista, TRIM(l.Nome) nome, TRIM(l.NomeFornec) forn, i.Codigobarra cod, TRIM(it.Descricao) descricao, it.qtdemb emb, i.l1,i.l2,i.l3,i.l4,i.l5,i.l6, TRIM(g.Descricao) grupo, TRIM(gs.Descricao) subgrupo
                           ...
                           LEFT JOIN central.gruposub gs ON gs.CodSubGrupo=it.CodGrupoSub LEFT JOIN central.grupo g ON g.CodGrupo=gs.CodGrupo WHERE it.CodDesativado=0`);
    const codsLista = [...new Set(itens.map(x => x.cod))], setLista = new Set(codsLista);
    // produtos FORA das listas de compra (Tiago, 18/09): todo item ativo do cadastro que não está em lista nenhuma;
    // só entra a loja onde tem estoque > 0 (sem estoque não há o que avaliar). Histórico de venda calculado aqui
    // pelo mesmo motor do Radar (calcularHist12), porque o hist24 do Radar só cobre os produtos das listas.
    let foraItens = [];
    try {
      const todos = await q(`SELECT it.CodigoBarra cod, TRIM(it.Descricao) descricao, it.qtdemb emb, TRIM(g.Descricao) grupo, TRIM(gs.Descricao) subgrupo
                             FROM central.itens it LEFT JOIN central.gruposub gs ON gs.CodSubGrupo=it.CodGrupoSub LEFT JOIN central.grupo g ON g.CodGrupo=gs.CodGrupo WHERE it.CodDesativado=0`);
      foraItens = todos.filter(x => !setLista.has(x.cod)).map(x => ({ ...x, lista: null, nome: null, forn: null, fora: true }));
    } catch (e) { console.error('[SORTIMENTO] itens fora das listas:', e.message); }
    const cods = [...new Set(codsLista.concat(foraItens.map(x => x.cod)))];
    ...
    const est = {}, custo = {}, ent = {}, preco = {}, margem = {}, ultCompra = {}, ultCompraErp = {};
    const pp = v => { const n = parseFloat(String(v ?? '').replace(',', '.')); return isFinite(n) ? n : 0; };
    for (const ln of LOJAS) { est[ln] = {}; custo[ln] = {}; preco[ln] = {}; margem[ln] = {}; ultCompraErp[ln] = {}; for (const ch of chunk(cods, 4000)) { const ph = ch.map(() => '?').join(',');
      ...
      for (const r of await q(`SELECT CodigoBarra cod, Custo, UltimaCompra FROM central.custoloja${ln} WHERE CodigoBarra IN (${ph})`, ch).catch(() => [])) { custo[ln][r.cod] = num(r.Custo); const d = isoDate(r.UltimaCompra); if (d) ultCompraErp[ln][r.cod] = d; } } }
    ...
    // data exata da última venda: preenchida depois de montar as linhas, consultando o mês da última venda de cada item×loja
    const ultDia = {};
    // fora das listas: só loja com estoque; histórico de venda 24 m calculado agora pelo motor do Radar
    foraItens = foraItens.filter(x => LOJAS.some(ln => (est[ln][x.cod] || 0) > 0));
    let HF = {};
    if (foraItens.length && deps.radar && deps.radar.calcularHist12) {
      try { const hf = await deps.radar.calcularHist12(foraItens.map(x => x.cod), hoje.toISOString().slice(0, 10)); HF = hf.mensal || {}; console.log(`[SORTIMENTO] fora das listas: ${foraItens.length} produtos com estoque, histórico calculado`); }
      catch (e) { console.error('[SORTIMENTO] histórico fora das listas:', e.message); }
    }
    const alvos = [];
    for (const it of itens) for (const ln of LOJAS) if (it['l' + ln]) alvos.push([it, ln, false]);
    for (const it of foraItens) for (const ln of LOJAS) if ((est[ln][it.cod] || 0) > 0) alvos.push([it, ln, true]);
    const rows = [];
    for (const [it, ln, fora] of alvos) {
      const h = (fora ? HF : H)[`${it.cod}|${ln}`] || {}; const meses = Object.entries(h).filter(([, x]) => x[0] > 0);
      ...
      const consumo = CONSUMO_RE.test(it.grupo || '') || CONSUMO_RE.test(it.subgrupo || '') || CONSUMO_RE.test(it.nome || '') || /^(CONSUMO|USO INTERNO|MAT\.? ?(DE )?(CONSUMO|LIMPEZA|ESCRIT))\b/i.test(it.descricao);
      let classe = 'ok';
      if (insumo) classe = 'insumo_producao';
      else if (v24 === 0 && en.q === 0) classe = fora ? 'parado' : (consumo ? 'consumo_sem_movimento' : 'marcado_sem_movimento');
      ...
      rows.push({ fora: !!fora, grupo: it.grupo || null, subgrupo: it.subgrupo || null, consumo, lista: it.lista, nome: it.nome, forn: it.forn, comprador: compradorPorLista[it.lista] || null, loja: ln, cod: it.cod, descricao: it.descricao, emb: num(it.emb) || 1,
        ...
        preco: +(preco[ln][it.cod] || 0).toFixed(2), margemCad: margem[ln][it.cod] ?? null, margemApl: (preco[ln][it.cod] > 0 && cu > 0) ? +(((preco[ln][it.cod] - cu) / cu) * 100).toFixed(2) : null,   // margem S/CUSTO, igual ao cadastro do ERP
        ultVendaDia: ultDia[`${it.cod}|${ln}`] || null,
        ultCompra: ultCompraErp[ln][it.cod] || (ultCompra[`${it.cod}|${ln}`] || {}).dt || en.ult || null, ultCompraPreco: (ultCompra[`${it.cod}|${ln}`] || {}).preco ?? null, ent24: +en.q.toFixed(3), ent6: +en.q6.toFixed(3), ultEnt: en.ult, cob, classe });
    }
    // dia exato da última venda (o histórico mensal só guarda o mês): pra cada loja × mês, MAX(Data) nos cupons daquele mês
    // (tabela ln{loja}mes{MM}, com Data no range do mês certo — a tabela do mês guarda mais de um ano)
    try {
      const grupos = {};
      for (const r of rows) if (r.ultVenda) (grupos[`${r.loja}|${r.ultVenda}`] = grupos[`${r.loja}|${r.ultVenda}`] || new Set()).add(r.cod);
      let nq = 0;
      for (const [g, set] of Object.entries(grupos)) {
        const [ln, ym] = g.split('|'); const [y, m] = ym.split('-'); const tb = `ln${ln}mes${m}`;
        const dIni = `${y}-${m}-01`, dFim = `${y}-${m}-${new Date(+y, +m, 0).getDate()}`;
        for (const ch of chunk([...set], 4000)) { const ph = ch.map(() => '?').join(','); nq++;
          for (const r of await q(`SELECT Codigo cod, DATE_FORMAT(MAX(Data),'%Y-%m-%d') d FROM \`${tb}\`.zcupomitens WHERE Data BETWEEN ? AND ? AND IndCancel='N' AND Codigo IN (${ph}) GROUP BY Codigo`, [dIni, dFim, ...ch]).catch(e => { console.error('[SORTIMENTO] última venda (dia)', tb, ym, e.message); return []; })) ultDia[`${r.cod}|${ln}`] = r.d; }
      }
      for (const r of rows) if (r.ultVenda) r.ultVendaDia = ultDia[`${r.cod}|${r.loja}`] || null;
      console.log(`[SORTIMENTO] última venda (dia): ${Object.keys(grupos).length} grupos loja×mês, ${nq} consultas, ${Object.keys(ultDia).length} itens×loja com dia`);
    } catch (e) { console.error('[SORTIMENTO] última venda (dia):', e.message); }
...
function filtrar({ loja, comprador, classe, lista, busca, faixas, ultDe, ultAte, consumo, escopo }) {
  if (!cache) return [];
  const b = (busca || '').toLowerCase();
  const fx = [];
  // escopo: 'listas' (padrão) = só produtos das listas de compra · 'fora' = só os que não estão em lista · 'todos'
  if (escopo === 'fora') fx.push(x => !!x.fora); else if (escopo !== 'todos') fx.push(x => !x.fora);
  // itens de consumo (uso interno) só entram com a flag "Consumo" ligada na tela (Tiago, 18/09)
  if (!consumo) fx.push(x => !x.consumo);
...
function resumo(rows) {
  const R = { itens: 0, estoque: 0, parado: 0, valorParado: 0, naoMerece: 0, valorNaoMerece: 0, semMov: 0, valorSemMov: 0, semMovConsumo: 0, valorSemMovConsumo: 0, cobExc: 0, valorCobExc: 0, compraNaoVende: 0, insumos: 0, porLoja: {}, porClasse: {} };
  for (const r of rows) {
    if (r.classe === 'insumo_producao') { R.insumos++; continue; }
    R.itens++; R.estoque += r.valorEst; R.porClasse[r.classe] = (R.porClasse[r.classe] || 0) + 1;
    const L = R.porLoja[r.loja] || (R.porLoja[r.loja] = { itens: 0, estoque: 0, parado: 0, valorParado: 0, naoMerece: 0, valorNaoMerece: 0, semMov: 0, valorSemMov: 0, semMovConsumo: 0, valorSemMovConsumo: 0, cobExc: 0, valorCobExc: 0 });
    ...
    if (r.classe === 'consumo_sem_movimento') { R.semMovConsumo++; R.valorSemMovConsumo += r.valorEst; L.semMovConsumo++; L.valorSemMovConsumo += r.valorEst; }
...
function csv(rows) {
  ...
  const CLASSE_PT = { cobertura_excessiva: 'Cobertura acima de 120 dias', parado: 'Parado', nao_merece: 'Não merece estar na loja', marcado_sem_movimento: 'Marcado sem movimento (venda)', consumo_sem_movimento: 'Sem movimento (item de consumo)', compra_e_nao_vende: 'Compra e não vende', ok: 'Normal', insumo_producao: 'Insumo de produção' };
  const head = 'Nº lista;Lista;Fornecedor;Comprador;Loja;Código;Descrição;Grupo;Subgrupo;Tipo;Embalagem;Venda 24 m (un);Venda 24 m (R$);Venda 12 m (un);Venda 6 m (un);Meses com venda (12 m);Última venda (mês);Última venda (dia);Estoque;Preço compra (custo);Preço venda;Margem cadastro (%);Margem s/ custo (%);Última compra;Preço última compra;Valor estoque;Entradas 24 m;Entradas 6 m;Última entrada;Cobertura (dias);Classe';
  return '﻿' + [head].concat(rows.map(r => [r.lista || '', r.nome || (r.fora ? '(fora das listas)' : ''), r.forn || '', r.comprador || '', 'L' + r.loja + ' ' + NOMES[r.loja], r.cod, r.descricao, r.fora ? 'FORA DAS LISTAS' : (r.grupo || ''), r.subgrupo || '', r.consumo ? 'Consumo' : 'Venda', r.emb, r.v24, r.r24, r.v12, r.v6, r.meses12, r.ultVenda || '', r.ultVendaDia || '', r.est, r.custo, r.preco, r.margemCad ?? '', r.margemApl ?? '', r.ultCompra || '', r.ultCompraPreco ?? '', r.valorEst, r.ent24, r.ent6, r.ultEnt || '', r.cob, CLASSE_PT[r.classe] || r.classe].map(esc).join(';'))).join('\r\n');
}
