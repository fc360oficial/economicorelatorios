...
function cruzar(erp, catalogo, cfg) {
  ...
  for (const r of erp) {
    ...
    // Unidade e embalagem (25/09/26): nem itens.Unid (todos "UN", até caixa de R$ 144) nem o
    // catálogo do app (Dlinks manda tudo "CX") são confiáveis. O que a CAHU escreve de verdade
    // é a descrição: "CX27", "CX C/9", "FD6", "PACK C2", "C32". Então a embalagem vem do nome;
    // sem marcação no nome = vende por unidade (UN), sem a linha "sai a R$ X a unidade".
    const { unidade, qtdEmbalagem } = embalagemDaDescricao(descr);
    out.push({
...
module.exports = { init, getConfig, salvarConfig, novoToken, tokenValido, baixarCatalogo, cruzar, embalagemDaDescricao, carregarProdutos, buscarProdutos, totais, montarSequencia, MODOS, TEMAS };
