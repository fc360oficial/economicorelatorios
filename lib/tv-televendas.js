...
function embalagemDoItem(r, descr) {
  ...
  if (und) {
    ...
    return { unidade: und, qtdEmbalagem: und === 'UN' ? 1 : Math.max(1, q), origemEmb: 'cadastro' };
  }
  return { ...embalagemDaDescricao(descr), origemEmb: 'nome' };
}
...
function cruzar(erp, catalogo, cfg) {
  ...
  for (const r of erp) {
    ...
    const { unidade, qtdEmbalagem, origemEmb } = embalagemDoItem(r, descr);
    out.push({
      ean, nome: descr, preco: Number(r.preco) || 0,
      unidade, qtdEmbalagem, origemEmb,
