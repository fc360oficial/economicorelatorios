...
// ── Embalagem pela descrição do ERP ─────────────────────────────────────────
// "NESCAU 180ML CX27" → CX/27 · "DOWNY 1,5L CX C/9" → CX/9 · "SPRITE 350ML FD6" → FD/6
// "ANTARCTICA PACK C2 GUARANA" → PACK/2 · "ALWAYS CX 8PCT C32" → CX/8 (primeira marcação)
// "ALA LAVA ROUPAS 400G" → UN/1. Marcação sem número ("CX" solto) = CX/1.
const RX_EMB_N = /\b(CX|CAIXA|FD|FARDO|PACK|PCT|PC|DP|DISPLAY|ENG|ENGRADADO)\s*\/?\s*(?:C\/|C|COM)?\s*(\d{1,3})(?!\d)/i;   // com quantidade: CX27, CX/12, CX C/9, CX 8PCT, PACK C2
const RX_EMB_S = /\b(CX|CAIXA|FD|FARDO|PACK)\b/i;                                                                  // marcação solta
const NOME_EMB = { CAIXA: 'CX', FARDO: 'FD', DISPLAY: 'DP', ENGRADADO: 'ENG' };
function embalagemDaDescricao(descr) {
  const d = String(descr || '');
  const n = RX_EMB_N.exec(d);   // marcação com número vale mais, em qualquer posição ("PACK ... FD6" → FD/6)
  if (n) { const t = n[1].toUpperCase(); return { unidade: NOME_EMB[t] || t, qtdEmbalagem: Math.max(1, parseInt(n[2], 10)) }; }
  const f = /\bF(\d{1,3})\s*$/i.exec(d);   // "GUARDANAPO MALU 21X23CM F48" → fardo de 48 (só no fim do nome)
  if (f) return { unidade: 'FD', qtdEmbalagem: Math.max(1, parseInt(f[1], 10)) };
  const s = RX_EMB_S.exec(d);
  if (s) { const t = s[1].toUpperCase(); return { unidade: NOME_EMB[t] || t, qtdEmbalagem: 1 }; }
  return { unidade: 'UN', qtdEmbalagem: 1 };
}

// Linha do ERP já vem com as colunas de central.embalagempadrao_venda (ev.*). A coluna da
// unidade de venda tem nome que não conhecemos de cor (Und_venda / UndVenda / Unidade...):
// acha por padrão, ignorando a Unid do cadastro geral (não vem na query).
function embalagemDoItem(r, descr) {
  const kUnd = Object.keys(r).find(k => /^(und|unid)/i.test(k) && k !== 'Unid');
  const und = kUnd ? String(r[kUnd] || '').trim().toUpperCase() : '';
  if (und) {
    const q = Math.round(Number(r.Qtd_venda) || 0);
    // "X" (emb_multipla = 1): o preço da tabela é POR UNIDADE e a venda é em múltiplos da
    // embalagem (ALA: R$ 2,30 a unidade, caixa fechada de 27). Sem X: preço é da embalagem.
    const multiplo = und !== 'UN' && Number(r.emb_multipla) === 1;
    return { unidade: und, qtdEmbalagem: und === 'UN' ? 1 : Math.max(1, q), multiplo, origemEmb: 'cadastro' };
  }
  return { ...embalagemDaDescricao(descr), multiplo: false, origemEmb: 'nome' };
}
...
function cruzar(erp, catalogo, cfg) {
  ...
  for (const r of erp) {
    ...
    // Unidade e embalagem (25/09/26): fonte oficial é a tela do Dlinks Distribuição > Cadastro
    // Embalagem Vendas (central.embalagempadrao_venda: Und. Venda + Qtd. Emb.). Item sem cadastro
    // lá cai na marcação do nome ("CX27", "CX C/9", "FD6", "PACK C2"); sem nada = UN.
    // itens.Unid (tudo "UN") e o catálogo do app (Dlinks manda tudo "CX") não servem.
    const { unidade, qtdEmbalagem, multiplo, origemEmb } = embalagemDoItem(r, descr);
    out.push({
      ean, nome: descr, preco: Number(r.preco) || 0,
      unidade, qtdEmbalagem, multiplo, origemEmb,
...
const SQL_RETIRADA = `
  SELECT s.codigobarra, s.descricao, s.preco, e.Qtd, ev.*
  ...
  LEFT JOIN central.embalagempadrao_venda ev ON ev.Codigobarra = s.codigobarra
...
module.exports = { init, getConfig, salvarConfig, novoToken, tokenValido, baixarCatalogo, cruzar, embalagemDaDescricao, embalagemDoItem, carregarProdutos, buscarProdutos, totais, montarSequencia, MODOS, TEMAS };
