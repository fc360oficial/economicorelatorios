...
//
// Hoje cada lista de compra do ERP (central.c_cotacao_lista) é de um fornecedor e tem uma compradora
// (central.c_cotacao_agenda_comprador). A ideia aqui é SIMULAR a divisão por categoria: uma relação
// "compradora → grupos" (ex.: PATRICIA → BEBIDAS) e, olhando TODAS as listas, o que cada compradora
...
// Relação de EXEMPLO: cada grupo vai pra compradora que já compra a maior parte dele hoje
...
// Simula a divisão. Pra cada compradora da relação: as listas onde existe item dos grupos dela.
...
// E, por lista, a quebra: quantos itens iriam pra cada compradora e quantos ficam sem dona na relação.
...
// CSV (separador ;) da visão por compradora — abre direto no Excel em pt-BR
function csv(resultado) {
  ...
  const linhas = [['Compradora', 'Grupos da compradora', 'Nº lista', 'Lista (nome sugerido)', 'Fornecedor', 'Compradora hoje', 'Itens do grupo', 'Itens da lista', '%', 'Situação'].join(';')];
