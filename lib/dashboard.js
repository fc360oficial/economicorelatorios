...
async function financeiro(hoje) {
  ...
  const linhasMap = {}, candidatos = [], alem = { n: 0, v: 0 }, vencidos = { n: 0, v: 0 }, pagarHoje = { n: 0, v: 0 }, semana = { n: 0, v: 0 }, proxima = { n: 0, v: 0 };
  ...
  candidatos.sort((a, b) => b.valor - a.valor); const top = candidatos.slice(0, 40); const nomeFornec = {};
  if (top.length) {
  ...
  return { hoje: dHoje, semIni, semFim, proxIni, proxFim, regra: 'sáb, dom e seg pagam na segunda · ter a sex pagam no próprio dia',
    ...
    maiores: top.map(m => ({ loja: m.loja, fornecedor: nomeFornec[m.nReg] || '', doc: m.doc, venc: m.venc, pag: m.pag, valor: m.valor })), saldo: null };
}
