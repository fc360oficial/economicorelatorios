...
async function calcular() {
  ...
  calculando = (async () => {
    ...
    const novo = { atualizadoEm: new Date().toISOString(), hoje: iso(hoje), _fat: fat, _grupos: gr,
      faturamento: fat, comercial: fat ? { ...comercial(fat, gr), compradores: await safe('compradores', () => compradores(hoje), null) } : null, abastecimento: abastecimento(),
...
async function segmento(nome) {
  ...
  segCalc[nome] = (async () => {
    ...
    if (nome === 'faturamento') out.faturamento = await faturamento(hoje);
      ...
      const gr = await safe('grupos', () => margemGrupos(hoje), { rows: [] });
      out.faturamento = fat; out.comercial = fat ? { ...comercial(fat, gr), compradores: await safe('compradores', () => compradores(hoje), null) } : null;
