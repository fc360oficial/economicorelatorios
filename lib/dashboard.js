...
async function faturamento(hoje) {
  ...
  for (const ln of LOJAS) {
    ...
    lojas.push({ loja: ln, nome: NOMES[ln], venda: r2(v), custo: r2(c), cupons: n, ticket: n ? r2(v / n) : 0, margem: pct(v - c, v), markup: pct(v - c, c),
      vendaAnt: r2(vAnt), custoAnt: r2(cAnt), cuponsAnt: nAnt, ticketAnt: nAnt ? r2(vAnt / nAnt) : 0, margemAnt: pct(vAnt - cAnt, vAnt), var: vAnt > 0 ? r1((v / vAnt - 1) * 100) : null,
  ...
  const rede = { loja: 0, nome: 'REDE', venda: r2(t.venda), custo: r2(t.custo), cupons: t.cupons, ticket: t.cupons ? r2(t.venda / t.cupons) : 0, margem: pct(t.venda - t.custo, t.venda), markup: pct(t.venda - t.custo, t.custo),
    vendaAnt: r2(t.vendaAnt), cuponsAnt: t.cuponsAnt, ticketAnt: t.cuponsAnt ? r2(t.vendaAnt / t.cuponsAnt) : 0, margemAnt: pct(t.vendaAnt - t.custoAnt, t.vendaAnt), var: t.vendaAnt > 0 ? r1((t.venda / t.vendaAnt - 1) * 100) : null,
...
function abastecimento() {
  ...
  if (rp) {
    try { const c = rp.curvaARisco(rp.TETO_PADRAO, null, undefined, true);
      ...
      out.curvaA = { resumo: c.resumo, itens: itens.slice(0, 15).map(i => ({ cod: i.cod, descricao: i.descricao, rank: i.rank, venda_dia_valor: i.venda_dia_valor, estoque: i.estoque, cobertura_dias: i.cobertura_dias, lojas_zeradas: (i.lojas_zeradas || []).map(x => x.loja), lista: i.lista, lista_nome: i.lista_nome, fornecedor: i.fornecedor, comprador: i.comprador, acao: i.acao, fazer_em: i.fazer_em })),
        transferir: (c.itens || []).filter(i => i.acao === 'transferir').slice(0, 20).map(i => ({ cod: i.cod, descricao: i.descricao, lojas_zeradas: (i.lojas_zeradas || []).map(x => x.loja), lojas_com_estoque: i.lojas_com_estoque, lojas_det: i.lojas_det, venda_dia_valor: i.venda_dia_valor })) };
  ...
  if (deps.pedidosCd) {
    try { const pend = deps.pedidosCd.pendenciasCD(); const itens = Object.entries(pend).map(([k, v]) => ({ loja: +k.split('|')[1], unidade: k.split('|')[0], descricao: v.descricaoCD, faltaCx: v.faltaCx, pedidoId: v.pedidoId, criadoEm: v.criadoEm }));
      ...
      out.cd = { pendencias: itens.length, caixasFaltando: itens.reduce((s, i) => s + i.faltaCx, 0), abertos: abertos.length, atrasados: abertos.filter(p => new Date(p.criadoEm).getTime() < limite).length,
        itens: itens.sort((a, b) => b.faltaCx - a.faltaCx).slice(0, 12), pedidosAtrasados: abertos.filter(p => new Date(p.criadoEm).getTime() < limite).slice(0, 10).map(p => ({ id: p.id, loja: p.loja, status: p.status, criadoEm: p.criadoEm, itens: (p.itens || []).length })) };
  ...
  if (deps.pedidosFornecedor) {
    try { const ps = deps.pedidosFornecedor.listar().filter(p => !p.teste);
      ...
      out.fornecedores = { total: ps.filter(p => !['recebido', 'cancelado'].includes(p.status)).length, parados: parados.length,
        itens: parados.sort((a, b) => a.criadoEm.localeCompare(b.criadoEm)).slice(0, 12).map(p => ({ id: p.id, lojas: p.lojas || [], fornecedor: p.fornecedor || '', lista: p.lista_nome || p.lista, status: p.status, dias: Math.floor((Date.now() - new Date(p.criadoEm).getTime()) / 86400000), valor: valorDe(p) })) };
...
async function estoque(hoje) {
  ...
  if (deps.sortimento) {
    try { const rows = deps.sortimento.filtrar({ escopo: 'todos' }); const R = deps.sortimento.resumo(rows);
      ...
      out.excesso = { itens: R.cobExc, valor: r2(R.valorCobExc), parados: R.parado, valorParados: r2(R.valorParado), porLoja: R.porLoja, calculadoEm: deps.sortimento.estado().calculadoEm,
        top: exc.slice(0, 15).map(r => ({ loja: r.loja, cod: r.cod, descricao: r.descricao, est: r.est, valorEst: r.valorEst, cob: r.cob, v6: r.v6, lista: r.lista, nome: r.nome })) };
  ...
  try {
    ...
    out.vencimentos = { itens: n, valor: r2(valor), buckets, porLoja, top: top.sort((a, b) => b.valor - a.valor).slice(0, 15) };
...
async function financeiro(hoje) {
  ...
  const maiores = await q(`SELECT c.Filial loja, f.Nome fornecedor, c.nDoc doc, DATE_FORMAT(c.DataVencto,'%Y-%m-%d') d, c.Valor - IFNULL(b.pago,0) v, c.Historico hist
                           ...
                           WHERE c.DataVencto BETWEEN ? AND ? AND c.Valor - IFNULL(b.pago,0) > 0.01 ORDER BY v DESC LIMIT 12`, [dHoje, iso(d7)]).catch(() => []);
  ...
  return { vencidos: { n: vencidos.n, v: r2(vencidos.v) }, hoje: { n: hojeT.n, v: r2(hojeT.v) }, sete: { n: sete.n, v: r2(sete.v) }, dias: dias.map(d => ({ ...d, v: r2(d.v) })),
    ...
    maiores: maiores.map(m => ({ loja: +m.loja, fornecedor: m.fornecedor || m.hist || '', doc: m.doc, data: m.d, valor: r2(num(m.v)) })), saldo: null };
}
...
function agendar() { setTimeout(() => calcular().catch(e => console.error('[DASHBOARD]', e.message)), 20 * 1000); }

module.exports = { init, agendar, calcular, dados, getMetas, setMetas, NOMES };
