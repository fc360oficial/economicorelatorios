...
function sincronizarVinculos(produtosCD) {
  for (const p of produtosCD) {
    ...
    if (atual) { if (p.descricaoCD) atual.descricaoCD = p.descricaoCD; if (p.estoqueCx != null) atual.estoqueCx = p.estoqueCx; }
    if (atual && atual.status === 'confirmado') { if (atual.unPorCaixaCadastro !== p.unPorCaixa) { atual.unPorCaixaCadastro = p.unPorCaixa; } continue; }
    if (cod.length <= 13) {
      vinculos[cod] = { codigoCD: cod, unidade: cod, unPorCaixa: 1, unPorCaixaCadastro: p.unPorCaixa, origem: 'igual', status: 'confirmado', confirmadoPor: 'sistema', confirmadoEm: agora(), descricaoCD: p.descricaoCD || null, estoqueCx: p.estoqueCx ?? null };
      continue;
    }
    ...
    vinculos[cod] = { codigoCD: cod, unidade: null, unPorCaixa: (atual && atual.unPorCaixa) || p.unPorCaixa || null, unPorCaixaCadastro: p.unPorCaixa,
      origem: cand ? 'dun14' : null, status: cand ? 'sugerido' : 'pendente', candidato: cand, confirmadoPor: null, confirmadoEm: null, descricaoCD: p.descricaoCD || null, estoqueCx: p.estoqueCx ?? null };
  }
  gravarVinculos();
  return vinculos;
}
...
async function coletarCD() {
  ...
  sincronizarVinculos(cods.map(c => ({ codigoCD: c, unPorCaixa: cd[c].unPorCaixaCadastro, unidadeExiste: cand[c] || null, descricaoCD: cd[c].descricao, estoqueCx: cd[c].estoqueCx })));
  return cd;
}
