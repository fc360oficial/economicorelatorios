...
function calcularSugestao(b, vinc, cfg, transito) {
  ...
  for (const [cod, c] of Object.entries(b.cd)) {
    ...
    if (temVenda) {
      ...
      for (const ln of LOJAS) p.porLoja[ln] = { vq: un.porLoja[ln]?.vq || 0, est: un.porLoja[ln]?.est || 0, transito: transitoDe(transito, item.unidade, cod, ln) };
      ...
      for (const ln of LOJAS) { const cx = sobra > 0 ? 1 : 0; sobra -= cx; item.lojas[ln] = { cx, un: upc ? cx * upc : 0, vq: 0, est: un ? (un.porLoja[ln]?.est || 0) : 0, transito: transitoDe(transito, item.unidade, cod, ln), cobertura: null, zeraAntes: false }; }
    }
...
function transitoPedidos() {
  ...
  for (const p of listarPedidos())
    if ((p.status === 'aberto' || p.status === 'separado') && new Date(p.criadoEm).getTime() >= limite)
      ...
      for (const i of p.itens) { const k = `${i.unidade || i.codigoCD}|${p.loja}`; t[k] = (t[k] || 0) + i.unidades; }
  return t;
}
