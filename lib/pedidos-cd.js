...
function calcularSugestao(b, vinc, cfg, transito) {
  ...
  for (const [cod, c] of Object.entries(b.cd)) {
    ...
    if (temVenda) {
      ...
      for (const ln of LOJAS) p.porLoja[ln] = { vq: un.porLoja[ln]?.vq || 0, est: un.porLoja[ln]?.est || 0, transito: transito[`${item.unidade || cod}|${ln}`] || 0 };
      ...
      for (const ln of LOJAS) { const cx = sobra > 0 ? 1 : 0; sobra -= cx; item.lojas[ln] = { cx, un: upc ? cx * upc : 0, vq: 0, est: un ? (un.porLoja[ln]?.est || 0) : 0, transito: transito[`${item.unidade || cod}|${ln}`] || 0, cobertura: null, zeraAntes: false }; }
    }
