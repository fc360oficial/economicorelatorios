...
async function conferenciaLoja(p) {
  ...
  const itens = chaves.length ? await q(`/*conf-itens*/ SELECT chave, codigobarra cod, SUM(qtd) un, SUM(status=1) ok, COUNT(*) n FROM central.conferenciaitens WHERE chave IN (${chaves.map(() => '?').join(',')}) GROUP BY chave, codigobarra`, chaves) : [];
  ...
  for (const i of p.itens) {
    const cods = [i.recebidoComo, i.unidade, i.codigoCD].filter(Boolean); const un = cods.reduce((a, c) => a + (porCod[c] || 0), 0);
...
async function verificar() {
  ...
  for (const p of ordenados) {
    ...
    try {
      ...
      const linhasTodas = (codsCD.length && idsNota.length) ? await q(`/*nf-linhas*/ SELECT c.nNota, DATE_FORMAT(c.DataRecto,'%Y-%m-%d') d, cp.Item item, cp.CodigoBarra codLoja, cp.Qtd cx, cpc.CodigoBarra codCD
      ...
      if (notas.length || linhas.length) {
        ...
        for (const l of linhas) {
          ...
          porLinha[String(l.codCD)] = (porLinha[String(l.codCD)] || 0) + num(l.cx); comoLinha[String(l.codCD)] = String(l.codLoja);
        }
        for (const i of p.itens) {
          const porCodigo = (i.unidade && porCod[i.unidade]) || porCod[i.codigoCD] || 0;
          i.recebidas = porCodigo + (porLinha[i.codigoCD] || 0);
