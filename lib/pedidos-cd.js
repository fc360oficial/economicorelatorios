    ...
    try {
      if (!p.expedicao) {
        ...
        for (const k of cands) {
          const linhas = await q(`SELECT CodigoBarra cod FROM central.delivery_produtos WHERE nPedido=?`, [k.nPedido]);
          ...
          // precisa cobrir metade do pedido do app E metade do que o CD digitou (pedido avulso de 1 item não casa)
          if (batem === 0 || batem * 2 < p.itens.length || batem * 2 < cods.size) continue;
