  ...
  for (const p of ordenados) {
    ...
    try {
      if (!p.expedicao) {
        ...
        const filtroLoja = `nLoja=10 AND CodFornec IN (${ids.map(() => '?').join(',')})`;
        // DataEntrada é DATE e HoraEntrada é varchar HH:MM (pode ser nulo) — compara dia e, no mesmo dia, hora
        const filtroData = `(DataEntrada > ? OR (DataEntrada = ? AND COALESCE(HoraEntrada,'00:00') >= ?))`;
        const ped = ids.length ? await q(`SELECT nPedido, DATE_FORMAT(DataLiberacao,'%Y-%m-%d') d FROM central.painel_televendas WHERE ${filtroLoja} AND Status=4 AND ${filtroData}${excl} ORDER BY DataEntrada, HoraEntrada LIMIT 1`, [...ids, desdeDia, desdeDia, horaLocal, ...usadosArr]) : [];
        if (ped.length) {
          ...
          // ainda não liberado, mas o CD já digitou o pedido no Televendas (Status 0 digitado, 1 em separação,
          // 2 conferido) — só informativo na tela; o pedido segue 'aberto' até a liberação (Status 4)
          const pend = ids.length ? await q(`SELECT nPedido, Status statusCD, DATE_FORMAT(DataEntrada,'%Y-%m-%d') d, HoraEntrada hora FROM central.painel_televendas WHERE ${filtroLoja} AND Status<4 AND ${filtroData}${excl} ORDER BY DataEntrada, HoraEntrada LIMIT 1`, [...ids, desdeDia, desdeDia, horaLocal, ...usadosArr]) : [];
          if (pend.length) p.noCD = { nPedido: String(pend[0].nPedido), statusCD: num(pend[0].statusCD), data: pend[0].d, hora: pend[0].hora || null };
          else delete p.noCD;
        }
      }
