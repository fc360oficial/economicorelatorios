...
async function coletarCD() {
  ...
  for (const r of est) {
    ...
    if (c.length === 14) {
      ...
      cd[c] = { descricao: r.descricao, estoqueCx: qtd, estoqueUn: null, unPorCaixaCadastro: emb[c] || null, estoqueEm: 'cx' };
      ...
      cd[c] = { descricao: r.descricao, estoqueCx: qtd, estoqueUn: null, unPorCaixaCadastro: emb[c], estoqueEm: 'cx' };
    } else {
      ...
      cd[c] = { descricao: r.descricao, estoqueCx: Math.floor(qtd / upcUsado), estoqueUn: qtd, unPorCaixaCadastro: upc, estoqueEm: 'un' };
    }
  }
  ...
  for (const [lnS, itens] of Object.entries(lojas || {})) {
    ...
    for (const it of itens) {
      ...
      if (!v || v.status !== 'confirmado' || !v.unidade || !(v.unPorCaixa >= 1)) {
        ...
        its.push({ codigoCD: cod, unidade: null, semVinculo: true, descricao: c.descricao || cod, unPorCaixa: upc, caixas: cx, unidades: cx * upc, custoUn: 0, origem: 'novo', recebidas: 0, separadas: 0 });
        continue;
      }
...
module.exports = { coletarCD, init, getConfig, salvarConfig, getVinculos, sincronizarVinculos, salvarVinculo, removerVinculo, buscarUnidade, LOJAS, LOJAS_NOMES,
