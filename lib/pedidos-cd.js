...
const dtLocal = d => `${isoLocal(d)} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')}`;
...
async function verificar() {
  ...
  for (const p of listarPedidos()) {
    ...
    const desdeDT = dtLocal(criado);       // datetime local (ERP guarda horário local)
    const desdeDia = isoLocal(criado);     // dia local, pra granularidade de DATE
    const desdeDiaMargem = addDias(desdeDia, -1);
    try {
      if (!p.expedicao) {
        const cli = config.clientesLoja[p.loja];
        // nPedidos já usados por outros pedidos do app não podem casar de novo com este
        const usados = listarPedidos().filter(x => x.id !== p.id && x.expedicao).map(x => x.expedicao.nPedido);
        const excl = usados.length ? ` AND nPedido NOT IN (${usados.map(() => '?').join(',')})` : '';
        const ped = cli ? await q(`SELECT nPedido, DATE_FORMAT(DataLiberacao,'%Y-%m-%d') d FROM central.painel_televendas WHERE nLoja=10 AND CodFornec=? AND Status=4 AND CONCAT(DataEntrada,' ',HoraEntrada)>=?${excl} ORDER BY DataEntrada LIMIT 1`, [cli, desdeDT, ...usados]) : [];
        if (ped.length) {
      ...
      const notas = unids.length ? await q(`SELECT cp.CodigoBarra cod, SUM(cp.Qtd) cx, c.nNota, DATE_FORMAT(c.DataRecto,'%Y-%m-%d') d
        ...
        GROUP BY cp.CodigoBarra, c.nNota, c.DataRecto`, [p.loja, config.fornecedorCD, desdeDiaMargem, desdeDia, ...unids]) : [];
      if (notas.length) {
