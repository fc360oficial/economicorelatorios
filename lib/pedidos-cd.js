...
function sincronizarVinculos(produtosCD) {
  for (const p of produtosCD) {
    ...
    vinculos[cod] = { codigoCD: cod, unidade: null, unPorCaixa: (atual && atual.unPorCaixa) || p.unPorCaixa || null, unPorCaixaCadastro: p.unPorCaixa,
      origem, status: cand ? 'sugerido' : 'pendente', candidato: cand, candidatoDescricao: p.candidatoDescricao || null, alternativas: p.alternativas || [], confirmadoPor: null, confirmadoEm: null, descricaoCD: p.descricaoCD || null, estoqueCx: p.estoqueCx ?? null, estoqueUn: p.estoqueUn ?? null };
  }
  gravarVinculos();
  return vinculos;
}
...
async function coletarCD() {
  ...
  for (const r of est) {
    ...
    if (c.length === 14) {
      ...
      cd[c] = { descricao: r.descricao, estoqueCx: qtd, estoqueUn: null, unPorCaixaCadastro: emb[c] || null };
    } else {
      // código de unidade: estoque do CD está em UNIDADES; un/cx vem do cadastro (Embalagem Vendas, senão qtdemb, senão 1)
      const upc = emb[c] || (qtdemb > 1 ? qtdemb : 1);
      const v = vinculos[c]; const upcUsado = (v && v.status === 'confirmado' && v.unPorCaixa >= 1) ? v.unPorCaixa : upc;
      cd[c] = { descricao: r.descricao, estoqueCx: Math.floor(qtd / upcUsado), estoqueUn: qtd, unPorCaixaCadastro: upc };
    }
  }
  sincronizarVinculos(cods.map(c => ({ codigoCD: c, unPorCaixa: cd[c].unPorCaixaCadastro, unidadeExiste: cand[c] || null, candidatoDescricao: porDesc[c]?.candidato || null, alternativas: porDesc[c]?.alternativas || [], descricaoCD: cd[c].descricao, estoqueCx: cd[c].estoqueCx, estoqueUn: cd[c].estoqueUn })));
  return cd;
}
