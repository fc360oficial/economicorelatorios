...
function sincronizarVinculos(produtosCD) {
  for (const p of produtosCD) {
    ...
    const cand = p.unidadeExiste || (nota && nota.unidade) || p.candidatoDescricao || null;
    const origem = p.unidadeExiste ? 'dun14' : (nota && nota.unidade ? 'nota' : (p.candidatoDescricao ? 'descricao' : null));
    // descrição da unidade candidata, pra quem confere saber o que é sem abrir o ERP
    const descricaoCandidato = p.unidadeExiste ? (p.descricaoUnidadeExiste || null) : origem === 'nota' ? (nota.descricao || null) : (((p.alternativas || []).find(a => a.cod === cand) || {}).descricao || null);
...
function removerVinculo(codigoCD) {
  ...
  const origem = !atual.candidato ? null : atual.sugestaoNota && atual.candidato === atual.sugestaoNota.unidade ? 'nota' : atual.candidato === atual.candidatoDescricao ? 'descricao' : 'dun14';
...
function calcularSugestao(b, vinc, cfg, transito) {
  ...
  for (const [cod, c] of Object.entries(b.cd)) {
    ...
    if (temVenda) {
      ...
      for (const ln of LOJAS) { const tCD = transitoDe(transito, item.unidade, cod, ln), tF = un ? (un.porLoja[ln]?.transitoForn || 0) : 0; const cx = sobra > 0 && tCD + tF <= 0 ? 1 : 0; sobra -= cx; item.lojas[ln] = { cx, un: upc ? cx * upc : 0, vq: 0, est: un ? (un.porLoja[ln]?.est || 0) : 0, transito: tCD + tF, transitoCD: tCD, transitoForn: tF, transitoFornDet: un ? (un.porLoja[ln]?.transitoFornDet || []) : [], cobertura: null, zeraAntes: false }; }
    }
...
async function aprenderVinculosDaNota(loja, linhas) {
  ...
  for (const l of uteis) {
    ...
    const atual = v || { codigoCD: cod, unidade: null, unPorCaixa: null, unPorCaixaCadastro: null, alternativas: [], confirmadoPor: null, confirmadoEm: null };
    // código de barras (DUN-14 → EAN-13) continua com prioridade; nota vem antes da descrição
    if (atual.origem === 'dun14' && atual.candidato) { if (!atual.sugestaoNota || atual.sugestaoNota.unidade !== un) { atual.sugestaoNota = info; mudou = true; } vinculos[cod] = atual; continue; }
    if (atual.candidato === un && atual.origem === 'nota') continue;
    vinculos[cod] = { ...atual, candidato: un, descricaoCandidato: info.descricao, origem: 'nota', status: 'sugerido', sugestaoNota: info };
    mudou = true;
  }
...
const CONF_STATUS = { 1: 'em conferência na loja', 3: 'conferida pela loja, aguardando central', 4: 'conferida pela loja, aguardando central', 2: 'liberada pela central' };
async function conferenciaLoja(p) {
  ...
  const itens = chaves.length ? await q(`/*conf-itens*/ SELECT chave, codigobarra cod, SUM(qtd*COALESCE(NULLIF(qtdemb,0),1)) un, SUM(status=1) ok, COUNT(*) n FROM central.conferenciaitens WHERE chave IN (${chaves.map(() => '?').join(',')}) GROUP BY chave, codigobarra`, chaves) : [];
  const porCod = {}; const porChave = {};
  for (const it of itens) { const cod = String(it.cod); porCod[cod] = (porCod[cod] || 0) + num(it.un); const k = String(it.chave); porChave[k] = porChave[k] || { itens: 0, conferidos: 0 }; porChave[k].itens++; if (num(it.ok) > 0) porChave[k].conferidos++; }
  ...
  for (const i of p.itens) {
    ...
    if (un > 0) { i.conferidas = +(un / (i.unPorCaixa || 1)).toFixed(2); } else delete i.conferidas;
  }
...
async function verificar() {
  ...
  for (const p of ordenados) {
    if (p.status === 'recebido' && p.recebimento && !p.recebimento.fechada) {
      // já chegou: só acompanha a conferência da loja até a nota fechar (Status F no ERP)
      try { await conferenciaLoja(p); salvarPedido(p); } catch (e) { console.error('[PEDIDOS-CD] conferência pedido', p.id, e.message); }
      continue;
    }
    ...
    try {
      ...
      const notas = unids.length ? await q(`SELECT cp.CodigoBarra cod, SUM(cp.Qtd*COALESCE(NULLIF(cp.QtdEmb,0),1)) un, c.nNota, DATE_FORMAT(c.DataRecto,'%Y-%m-%d') d
      ...
      const linhasTodas = (codsCD.length && idsNota.length) ? await q(`/*nf-linhas*/ SELECT c.nNota, DATE_FORMAT(c.DataRecto,'%Y-%m-%d') d, cp.Item item, cp.CodigoBarra codLoja, cp.Qtd*COALESCE(NULLIF(cp.QtdEmb,0),1) un, cpc.CodigoBarra codCD
      ...
      // a loja também recebe notas avulsas do CD todo dia: pelo casamento de linhas só vale a nota que cobre pelo
      // menos metade dos itens do pedido (a nota de venda do CD é 1 por pedido, com todos os itens)
      const cobertura = {}; for (const l of linhasTodas) (cobertura[String(l.nNota)] = cobertura[String(l.nNota)] || new Set()).add(String(l.codCD));
      const linhas = linhasTodas.filter(l => cobertura[String(l.nNota)].size * 2 >= codsCD.length);
      if (notas.length || linhas.length) {
        const porCod = {}; const nn = new Map();
        for (const n of notas) { porCod[String(n.cod)] = (porCod[String(n.cod)] || 0) + num(n.un); nn.set(String(n.nNota), n.d); }
        ...
        for (const l of linhas) {
          ...
          porLinha[String(l.codCD)] = (porLinha[String(l.codCD)] || 0) + num(l.un); comoLinha[String(l.codCD)] = String(l.codLoja);
        }
        for (const i of p.itens) {
          // unidades → caixas (nota lançada pelo código de unidade traz Qtd em un; pelo de caixa, Qtd×QtdEmb)
          const porCodigo = (i.unidade && porCod[i.unidade]) || porCod[i.codigoCD] || 0;
          i.recebidas = +((porCodigo + (porLinha[i.codigoCD] || 0)) / (i.unPorCaixa || 1)).toFixed(2);
          if (porLinha[i.codigoCD]) i.recebidoComo = comoLinha[i.codigoCD]; else delete i.recebidoComo;
        }
        await aprenderVinculosDaNota(p.loja, linhas);
        p.recebimento = { notas: [...nn].map(([nNota, data]) => ({ nNota, data })), verificadoEm: agora() };
        await conferenciaLoja(p);
