...
async function conferenciaLoja(p) {
  ...
  for (const i of p.itens) {
    ...
    // o bipe da loja vem na MESMA unidade em que a loja lançou a nota (7 "fardos" ou 27 un): usa a escala da nota
    const escala = (i.recebidas > 0 && i.unNota > 0) ? i.recebidas / i.unNota : 1 / (i.unPorCaixa || 1);
    if (un > 0) { i.conferidas = +(un * escala).toFixed(2); } else delete i.conferidas;
  }
...
async function verificar() {
  ...
  for (const p of ordenados) {
    ...
    try {
      ...
      const notas = unids.length ? await q(`SELECT cp.CodigoBarra cod, SUM(cp.Qtd*COALESCE(NULLIF(cp.QtdEmb,0),1)) un, SUM(cp.Total) tot, c.nNota, DATE_FORMAT(c.DataRecto,'%Y-%m-%d') d
