...
async function verificar() {
  ...
  for (const p of ordenados) {
    ...
    try {
      ...
      // mesma regra do casamento por linha: a loja recebe notas avulsas do CD todo dia (19/09/2026: L5 grudou 5004 e 5019
      // com 7 e 1 itens em comum e inflou as caixas recebidas); só vale nota que cobre >= metade dos itens do pedido
      const cobCod = {}; for (const n of notasTodas) (cobCod[String(n.nNota)] = cobCod[String(n.nNota)] || new Set()).add(String(n.cod));
      const notas = notasTodas.filter(n => cobCod[String(n.nNota)].size * 2 >= unids.length);
      ...
      if (!notas.length && !linhas.length && p.recebimento && p.status !== 'finalizado') {
