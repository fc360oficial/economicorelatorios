...
async function coletarUnidades(codigos, hoje) {
  const { q, mesDB } = deps;
  // janela inclui HOJE (dia parcial): a primeira venda de um produto conta na hora e ele sai de "produto novo" no próximo
  // recálculo — Tiago, 22/09/2026: Fandangos vendeu 13 un na L1 no próprio dia e a tela dizia "sem venda".
  const dFim = hoje, dIni = addDias(hoje, -(JANELA_VENDA_DIAS - 1));
  ...
  // trânsito de FORNECEDOR: sugestão/pedido de compra em aberto no ERP (Status 0/1, últimos 30 d) — se o(a) comprador(a)
...
function calcularSugestao(b, vinc, cfg, transito, pendencias, enviadosNovo, recebidosNovo) {
  ...
  for (const [cod, c] of Object.entries(b.cd)) {
    ...
    // Estoque parado: a loja tem o produto no ERP mas não vende há 24+ meses (senão cairia em 'repor' pela média mensal).
    // Mantém a caixa, mas sinaliza pra conferir no ERP/na loja se esse estoque existe — pode ser pra zerar no ERP
    // (Tiago, 22/09/2026: Fandangos com 1.187 un na L1 e venda zero).
    if (item.origem === 'novo') { const parado = un ? LOJAS.filter(ln => (un.porLoja[ln]?.est || 0) > 0).map(ln => ({ ln, est: un.porLoja[ln].est })) : []; item.estoqueParado = parado.length ? parado : null; }
...
module.exports = { coletarCD, porTokens, transitoPedidos, pendenciasCD, init, getConfig, salvarConfig, getVinculos, sincronizarVinculos, salvarVinculo, removerVinculo, buscarUnidade, LOJAS, LOJAS_NOMES,
  ...
  criarPedidos, listarPedidos, obterPedido, cancelarPedido, verificar, _setBaseParaTeste, _coletarUnidadesParaTeste: coletarUnidades };
