...
// Substring pura (h.includes(op)) dava falso positivo: 'VERO' é substring de
// "SEVERO" (sobrenome comum), 'REDE' aparece dentro de nomes de empresa não
// relacionados. \b ancora em fronteira de palavra pra só casar a operadora
// como termo isolado.
const RE_OPERADORA_CARTAO = new RegExp('\\b(' + OPERADORAS_CARTAO.join('|') + ')\\b');

// Achado 04/09/2026 com extrato real: linhas como "SISPAG TICKET SERVICOS SA"
// e "SISPAG PLUXEE BENEFICIOS BRASIL S.A." (ex-Sodexo) são repasse/estorno de
// operadora de vale (voucher/benefício), não boleto/depósito avulso nem
// cartão de crédito/débito — categoria própria.
const OPERADORAS_VOUCHER = [
  'TICKET', 'PLUXEE', 'SODEXO', 'ALELO', 'VALECARD', 'GREENCARD', 'VEROCARD', 'VR '
];
const RE_OPERADORA_VOUCHER = new RegExp('\\b(' + OPERADORAS_VOUCHER.map(o => o.trim()).join('|') + ')\\b');

// Operadora de cartão é checada ANTES de deposito_boleto (mas não antes de
// pix_recebido, ver abaixo): um repasse de adquirente cujo histórico começa
// com "TED RECEBIDO"/"DEPOSITO" mas contém o nome da operadora (ex:
// "DEPOSITO CIELO LIQUIDACAO") é liquidação de cartão, não um depósito/boleto
// avulso — e cartão nunca pode entrar no pool de casamento linha a linha com
// fatura (ver comentário no topo de lib/conciliador-entradas.js).
// PIX é exceção a essa prioridade: é a categoria de maior risco de falso
// positivo, porque um nome de cliente/empresa real pode coincidentemente
// conter uma palavra da lista de operadoras (ex: "PIX RECEBIDO REDE
// SUPERMERCADOS BOM PRECO LTDA" é um pagamento de cliente de verdade, não
// liquidação da adquirente Rede) — uma liquidação de adquirente é uma fração
// muito menor do que aparece sob o prefixo PIX RECEBIDO do que sob os
// prefixos de depósito/TED/DOC/boleto. Por isso PIX RECEBIDO nunca vira
// 'cartao' por esse teste.
// Achado 04/09/2026 com extrato real via API: PIX recebido aparece como
// "PIX QRS ..." (QR estático) ou "PIX TRANSF ..." (transferência), nunca
// "PIX RECEBIDO" — como as três funções que chamam este classificador já
// filtram só valor positivo antes de chegar aqui, qualquer linha começando
// com "PIX " já é necessariamente um recebimento, então checar só o prefixo
// genérico "PIX " cobre qualquer variante de vocabulário sem precisar
// adivinhar cada uma.
// Achado 04/09/2026 com extrato real: liquidação de cartão traz um código
// logo antes do número de referência que diz o tipo — "CIELO MAST
// CD2836782573" (Crédito) vs "CIELO MAST DB2836782573" (Débito), "REDE ELO
// CD0087723786" (Crédito) — então dá pra separar débito de crédito no lado
// do banco, ao contrário do que se pensava antes (só o ERP tinha esse
// dado, e nem separado). Sem esse código no meio (adquirente/formato
// diferente do observado), cai em 'cartao' genérico — não inventa o tipo.
function subclassificarCartao(h) {
  if (/\bCD\d/.test(h)) return 'cartao_credito';
  if (/\bDB\d/.test(h)) return 'cartao_debito';
  return 'cartao';
}

function classificarEntrada(historico) {
  const h = historico.trim().toUpperCase();
  const ehPixRecebido = h.startsWith('PIX ');
  if (!ehPixRecebido && RE_OPERADORA_VOUCHER.test(h)) return 'voucher';
  if (!ehPixRecebido && RE_OPERADORA_CARTAO.test(h)) return subclassificarCartao(h);
  if (ehPixRecebido) return 'pix_recebido';
...
function extrairFavorecidoEntrada(historico, categoria, doc) {
  ...
  if (categoria === 'pix_recebido') resto = resto.replace(/^PIX \S+\s*/i, '');
...
// Achado 04/09/2026: `historico` (o que aparece na tela, montado em
// parseEntradasApi) prioriza `complete` sobre `shortened` — mas esta função
// priorizava o contrário, então classificava em cima de um texto mais curto
// que às vezes cortava o código CD/DB da liquidação de cartão (visível no
// `complete` exibido, ausente no `shortened` truncado), classificando como
// "cartao" genérico algo que na tela parecia ter o código. Corrigido pra
// mesma prioridade de `historico`, senão a categoria não bate com o texto
// que o usuário está vendo.
function classificarApiEntrada(literal) {
  const texto = (literal.complete || literal.shortened || '').trim().toUpperCase();
  const ehPixRecebido = texto.startsWith('PIX ');
  if (!ehPixRecebido && RE_OPERADORA_VOUCHER.test(texto)) return 'voucher';
  if (!ehPixRecebido && RE_OPERADORA_CARTAO.test(texto)) return subclassificarCartao(texto);
  if (ehPixRecebido) return 'pix_recebido';
