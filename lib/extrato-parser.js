...
// Operadora de cartão é checada ANTES de pix_recebido/deposito_boleto: um
// repasse de adquirente cujo histórico começa com "TED RECEBIDO"/"DEPOSITO"
// mas contém o nome da operadora (ex: "DEPOSITO CIELO LIQUIDACAO") é
// liquidação de cartão, não um depósito/boleto avulso — e cartão nunca pode
// entrar no pool de casamento linha a linha com fatura (ver comentário no
// topo de lib/conciliador-entradas.js).
function classificarEntrada(historico) {
  const h = historico.trim().toUpperCase();
  if (RE_OPERADORA_CARTAO.test(h)) return 'cartao';
...
function classificarApiEntrada(literal) {
  ...
  if (RE_OPERADORA_CARTAO.test(texto)) return 'cartao';
