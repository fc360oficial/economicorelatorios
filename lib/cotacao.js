...
//   3. comparativo fornecedor a fornecedor: o menor preço vence por item; o(a) comprador(a) pode trocar
...
function salvarPrecos(token, itens, extra) {
  ...
  for (const n of itens || []) {
    ...
    // embalagem informada pelo vendedor ("emb. correta?", igual ao pedido do Radar) — só observação, o(a) comprador(a) decide
...
// mensagem do(a) comprador(a) aos fornecedores (aparece fixa no topo da página do vendedor); só o(a) comprador(a) edita
function setMensagem(id, { mensagem, whats, nome }, usuario) {
  const c = obter(id); if (!c) return null;
  if (mensagem !== undefined) { c.mensagem = String(mensagem || '').trim().slice(0, 1500) || null; c.mensagemEm = c.mensagem ? new Date().toISOString() : null; c.mensagemPor = c.mensagem ? (usuario || null) : null; }
