...
function comparativo(c) {
  ...
  const forns = c.fornecedores.map(f => ({
    codFornec: f.codFornec, nome: f.nome, status: f.status, vendedor: f.vendedor, vendedores: f.vendedores || [], condicao: f.condicao, obs: f.obs, token: f.token,
...
// mensagem da compradora aos fornecedores (aparece fixa no topo da página do vendedor); só a compradora edita
...
module.exports = { init, criar, obter, listar, porToken, adicionarFornecedor, marcarEnviado, abrir, salvarPrecos, finalizar, visaoVendedor, setMensagem, prePedidos, salvarPrePedido, realizarPedido, comparativo, definirVencedor, fechar, cancelar, resumo, historicoProduto, marcarTeste, patchTeste, removerTestes, LOJAS_NOMES };
