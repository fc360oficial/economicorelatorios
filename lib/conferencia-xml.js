...
//
// Depois que a comprador(a) APROVA um pedido, ele fica "aguardando conferência XML".
...
function conferirLoja(p, ln, notas) {
  ...
  const embPedido = Object.fromEntries(p.itens.map(i => [i.cod, i.emb || 1]));
  for (const n of notas) for (const it of n.itens) {
    let unidades = it.unidades, conv = it.conversao || 'trib';
    if (conv === 'pendente') {
      const e = embPedido[it.cod];
      if (e && e > 1) { unidades = it.qtdCom * e; conv = 'emb'; }        // CX/FD sem tributável: converte pela embalagem do pedido
  ...
  for (const it of p.itens) {
    ...
    itens.push({ cod: it.cod, descricao: it.descricao, pedida, recebida, dif: +(recebida - pedida).toFixed(3), tipo, preco_digitado: precoDig, preco_xml: precoXml, preco_dif: precoDif, preco_pct: precoPct != null ? +(precoPct * 100).toFixed(2) : null, preco_status: precoStatus, xml: r ? r.xml : [], conferir_unidade: conferir });
  ...
  for (const p of listar()) {
    ...
    try {
      rows = await qERP(`/*entrada-nota*/ SELECT nt.nCompra, nt.nLoja, nt.nNota, nt.chave, nt.Status st, DATE_FORMAT(nt.DataRecto,'%Y-%m-%d') dr, nt.NomeOperador op,
          cf.Status cst, cf.OperadorLoja opLoja, cf.OperadorCentral opCentral, DATE_FORMAT(cf.DataEntrada,'%Y-%m-%d') de, cf.HoraEntrada he, DATE_FORMAT(cf.DataLiberacao,'%Y-%m-%d') dl, cf.HoraLiberacao hl
        FROM central.compras nt LEFT JOIN central.conferencia cf ON cf.nReg=nt.nConferencia
        WHERE nt.Movimentacao='COMPRA' AND nt.Status<>'C' AND (nt.chave IN (${todas.map(() => '?').join(',')})
      ...
      if (!h) { delete n.entrada; }
        ...
        n.entrada = { nCompra: h.nCompra, status: h.st || null, dataRecto: h.dr || null, operador: h.op || null,
          conferencia: h.cst == null ? null : { status: num(h.cst), texto: CONF_STATUS[num(h.cst)] || ('status ' + h.cst), operadorLoja: h.opLoja || null, operadorCentral: h.opCentral || null,
            entrada: h.de ? h.de + (h.he ? ' ' + String(h.he).slice(0, 5) : '') : null, liberacao: h.dl ? h.dl + (h.hl ? ' ' + String(h.hl).slice(0, 5) : '') : null } };
      }
    ...
    for (const x of Object.values(p.xml.lojas)) {
      ...
      else { x.chegou = { em: com.map(n => n.entrada.dataRecto).filter(Boolean).sort()[0] || null, notas: com.length, de: (x.notas || []).length, fechada: com.length === (x.notas || []).length && com.every(n => n.entrada.status === 'F') }; chegaram++; }
...
module.exports = { init, conferirTodos, verificarEntradas, conferirLoja, buscarNotas, LOJA_CNPJ, TESTE_DIR };
