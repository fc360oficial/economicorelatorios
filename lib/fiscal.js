...
function anexarEconomico(lista, de, ate, cfg) {
  const rec = depsF.recebimento; if (!rec || !lista.length) return lista;
  let cfgRec = null; try { cfgRec = rec.config(); } catch (e) { cfgRec = null; }
...
async function listar({ de, ate, loja }) {
  ...
  anexarEconomico(lista, de, ate, cfg);
  return { de, ate, loja: loja || null, config: cfg, erro_erp: erro, recebimentos: lista.map(resumo), gerado_em: new Date().toISOString() };
}
function resumo(r) {
  const { itens, ...resto } = r;
  // a lista não precisa dos itens bipados do coletor (o detalhe traz) — só do resumo da conferência
  if (resto.economico) { const { itens: _bip, ...eco } = resto.economico; resto.economico = eco; }
  return { ...resto, itens_com_flag: itens.filter(i => i.flags.length).length };
}
...
async function detalhe(nReg) {
  ...
  return anexarEconomico([cruzar(c, fatiar(bruto, c), cfg, dec[nReg])], conf.DataEntrada, conf.DataEntrada, cfg)[0];
}
...
module.exports = { init, pedidoAppPorCod, flagDecisao, listar, detalhe, documentos, margemItens, contatos, textoReconferencia, decidir, getDecisoes, getConfig, setConfig, criarTestes, removerTestes, temTestes, testesLista, LOJAS, LOJAS_NOMES, STATUS_ERP, STATUS_NOMES, CONFIG_PADRAO, cruzar, mesclarEconomico, conferenciaFisica, acharConf };
