...
// ── COLETOR ECONÔMICO (nosso PWA) × conferência do ERP ───────────────────────
// A conferência cega que a loja faz no nosso coletor (lib/recebimento.js) é uma
// SEGUNDA contagem, independente do coletor do ERP. Aqui ela entra na linha do
// Fiscal: itens bipados, devoluções (3 origens: compras / coletor / falta) e a
// "conferência física" (bipado × nota). A linha só fica 'pronto' quando pedido,
// XML e físico batem — qualquer um fora vira 'excecao' com o motivo.
const ECO_FECHADA = ['terminada', 'liberada'];
function conferenciaFisica(row, conf, devolucoes) {
  ...
  for (const cod of [...new Set([...Object.keys(nota), ...Object.keys(bip)])]) {
    ...
    const doRow = (row.itens || []).find(i => String(i.ean) === cod);
    diferencas.push({ cod, descricao: (doRow && doRow.descricao) || (conf.itens[cod] || {}).descricao || cod, nota: n, fisico: f, dif, tipo: !f ? 'nao_bipado' : !n ? 'nao_na_nota' : dif < 0 ? 'falta' : 'sobra' });
  }
  return { nivel: diferencas.length ? 'erro' : 'ok', diferencas,
    msg: diferencas.length ? `${diferencas.length} item(ns) com diferença entre o físico bipado e a nota` : `Contagem física bate com a nota (${Object.keys(bip).length} itens bipados)` };
}
// pura (testada em test/fiscal-recebimento.test.js): mescla UMA conferência nossa na linha do Fiscal
function mesclarEconomico(row, conf, devolucoes) {
  if (!row || !conf) return row;
  const dev = devolucoes || conf.devolucoes || [];
  const fisico = conferenciaFisica(row, conf, dev);
  const erp = conf.erp || {};
  const eco = {
  ...
  if (fisico.nivel === 'erro') row.veredito = 'divergente';
  return row;
}
function confsDoPeriodo(de, ate) {
  const rec = depsF.recebimento; if (!rec || !rec.listarDia) return [];
  ...
  for (let n = 0; d <= fim && n < 62; n++, d = addDias(d, 1)) { try { out.push(...rec.listarDia(d)); } catch (e) {} }
  return out;
}
function acharConf(row, confs) {
  const porNReg = confs.find(c => c.erp && c.erp.nReg != null && String(c.erp.nReg) === String(row.nReg));
  if (porNReg) return porNReg;
  const nums = new Set((row.notas || []).map(n => String(n.nNota)).filter(n => n && n !== 'null'));
  return confs.find(c => +c.loja === +row.loja && nums.has(String(c.nNota))) || null;
}
function anexarEconomico(lista, de, ate) {
  const rec = depsF.recebimento; if (!rec || !lista.length) return lista;
  let confs = []; try { confs = confsDoPeriodo(de, ate); } catch (e) { console.error('[FISCAL] coletor:', e.message); return lista; }
  for (const r of lista) {
    const c = acharConf(r, confs); if (!c) continue;
    let dev = c.devolucoes || [];
    if (!dev.length) { try { dev = rec.devolucoes(c); } catch (e) { dev = []; } }
    mesclarEconomico(r, c, dev);
  }
  return lista;
}
...
async function detalhe(nReg) {
  ...
  return anexarEconomico([cruzar(c, fatiar(bruto, c), cfg, dec[nReg])], conf.DataEntrada, conf.DataEntrada)[0];
}
...
module.exports = { init, pedidoAppPorCod, flagDecisao, listar, detalhe, documentos, margemItens, contatos, textoReconferencia, decidir, getDecisoes, getConfig, setConfig, criarTestes, removerTestes, temTestes, testesLista, LOJAS, LOJAS_NOMES, STATUS_ERP, STATUS_NOMES, CONFIG_PADRAO, cruzar, mesclarEconomico, conferenciaFisica };
