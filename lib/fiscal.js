...
let qERP = null;
function init(q) { qERP = q; fs.mkdirSync(DIR, { recursive: true }); }
...
function cruzar(conf, partes, cfg, decisao) {
  ...
  checks.pedido = !ehCompra ? { nivel: 'ok', msg: 'Não é compra (' + [...new Set(notasOut.map(n => n.movimentacao).filter(Boolean))].join(', ') + ')' }
    : comPedido ? { nivel: nPrecoPed ? 'erro' : nivelDe(nQtdPed, true), msg: nPrecoPed ? `${nPrecoPed} item(ns) com preço acima do pedido` : nQtdPed ? `${nQtdPed} item(ns) com quantidade ≠ pedido` : 'Pedido bate com a nota' }
...
function criarTestes() {
  ...
  function cen({ loja, fornecedor, status, hora, itens, pedido, boletos, semXml, obs, coletorFn, semColetor, precoFn, movimentacao, validadeFn, extraProds, extraColetor, decisao, opLoja, opCentral }) {
    ...
    const coletor = semColetor ? [] : (itens || prodsBase).map((p, i) => ({ chave: String(nReg), codigobarra: p.ean, emb: 'UN', qtd: String(coletorFn ? coletorFn(p, i) : (p.qtd || 24)), qtdemb: '1.000', status: 1, Reconferir: 0, DataValidade: validadeFn ? validadeFn(p, i) : (p.validade ? em(Math.round(p.validade * 0.8)) : null) })).concat(extraColetor ? extraColetor(nReg) : []);
    ...
    const x = {
      ...
      cadastro: prodsBase.map(p => ({ CodigoBarra: p.ean, Descricao: p.desc, qtdemb: String(p.emb), Unid: 'UN', Preco: String(p.venda), custo: String(p.custo), UltimoCusto: String(p.custo), margem: String(r2((p.venda - p.custo) / p.venda * 100)), Validade: p.validade, CodDesativado: 0 })),
  ...
  const lista = [
    ...
    cen({ loja: 3, fornecedor: F('E · VALIDADE CURTA'), status: 3, hora: '09:02:30', pedido: {}, validadeFn: (p, i) => i === 2 ? em(12) : i === 0 ? null : i === 4 ? em(5) : em(200), opLoja: 'MAYRA' }),
...
module.exports = { init, listar, detalhe, documentos, margemItens, contatos, textoReconferencia, decidir, getDecisoes, getConfig, setConfig, criarTestes, removerTestes, temTestes, testesLista, LOJAS, LOJAS_NOMES, STATUS_ERP, STATUS_NOMES, CONFIG_PADRAO, cruzar };
