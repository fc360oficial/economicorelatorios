...
// fornecedor + loja e aprovado nos últimos 45 dias. Traz a observação da compradora pro Fiscal.
...
function pedidoAppPorCod(codFornec, loja, chaves) {
  let lista = []; try { lista = (depsF.pedidos && depsF.pedidos()) || []; } catch (e) { return { map: {}, ids: [] }; }
  const setCh = new Set(chaves || []); const map = {}; const ids = [];
  for (const p of lista) {
    if (p.teste || !['aprovado', 'recebido', 'recebido_parcial'].includes(p.status)) continue;
    ...
    for (const i of x.itens || []) {
      ...
      map[cod] = { qtd_ped: +i.pedida || 0, qtd_xml: +i.recebida || 0, preco_ped: i.preco_digitado != null ? +i.preco_digitado : 0, preco_xml: i.preco_xml != null ? +i.preco_xml : 0, und_ped: unidDe(cod), und_xml: null,
        diferenca: i.preco_digitado != null && i.preco_xml != null ? r2((+i.preco_xml - +i.preco_digitado) * (+i.recebida || 0)) : 0, nPedido: null, app: p.id };
    }
    for (const i of x.nao_pedidos || []) { const cod = String(i.cod || '').trim(); if (cod && !map[cod]) map[cod] = { qtd_ped: 0, qtd_xml: +i.recebida || 0, preco_ped: 0, preco_xml: i.preco_xml != null ? +i.preco_xml : 0, und_ped: null, und_xml: null, diferenca: 0, nPedido: null, app: p.id, nao_pedido: true }; }
  }
  return { map, ids };
}
...
function cruzar(conf, partes, cfg, decisao) {
  ...
  let pedApp = { map: {}, ids: [] };
  if (!Object.keys(pedPorCod).length) { pedApp = pedidoAppPorCod(+conf.CodFornec, loja, chavesTodas); for (const [cod, pd] of Object.entries(pedApp.map)) if (!pd.nao_pedido) pedPorCod[cod] = pd; }
