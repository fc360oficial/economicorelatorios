...
function cruzar(conf, partes, cfg, decisao) {
  ...
  checks.pedido = !ehCompra ? { nivel: 'ok', msg: 'Não é compra (' + [...new Set(notasOut.map(n => n.movimentacao).filter(Boolean))].join(', ') + ')' }
    : comPedido ? { nivel: nPrecoPed ? 'erro' : nivelDe(nQtdPed, true), msg: (nPrecoPed ? `${nPrecoPed} item(ns) com preço acima do pedido` : nQtdPed ? `${nQtdPed} item(ns) com quantidade ≠ pedido` : 'Pedido bate com a nota') + (pedApp.ids.length ? ` · pedido de compra ${pedApp.ids.map(i => '#' + i).join(', ')} (app, preço digitado pelo vendedor)` : '') + (pedApp.aceitos || []).map(a => ` · consistência aceita${a.por ? ' por ' + a.por : ''}${a.em ? ' em ' + ddmm(a.em) : ''}${a.motivo ? ': ' + a.motivo : ''}`).join('') }
