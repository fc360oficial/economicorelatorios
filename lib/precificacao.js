...
const precifErp = require('./precificacao-erp');

let DIR = path.join(__dirname, '..', 'data', 'precificacao');
let qERP = null, radar = null, qTeste = null;   // qTeste: SELECT no MySQL de teste (.254), pra conferir lista enviada pra carga lá
let padrao = { politica: 'padrao', arredondamento: '9', ajuste_centavos: '' };   // ajuste_centavos: "89>99,09>19" (flag na tela) ou '' = desligado   // politica é legado: regra única (ver precificacao-calc.js)

function init(opts = {}) { if (opts.dir) DIR = opts.dir; fs.mkdirSync(DIR, { recursive: true }); }
function initERP(q, radarMod, qT) { qERP = q; radar = radarMod || null; qTeste = qT || null; }
function setPadrao(p) { padrao = { politica: calc.POLITICAS.includes(p?.politica) ? p.politica : padrao.politica, arredondamento: calc.TERMINACOES.includes(p?.arredondamento) ? p.arredondamento : padrao.arredondamento, ajuste_centavos: p?.ajuste_centavos != null && calc.validaAjuste(String(p.ajuste_centavos).trim()) ? String(p.ajuste_centavos).trim() : (padrao.ajuste_centavos || '') }; return padrao; }
...
function rateioDe(x) {
  ...
  // Impostos (IPI/ST), frete e desconto ficam guardados só pra informação: NÃO mexem no preço.
  // O custo novo já chega com imposto (decisão do Tiago, 15/09/2026 — antes a regra acrescentava
  // IPI+ST sobre o valor dos produtos, o que duplicava o imposto).
  return { ...s, fator: 0, disponivel };
}

// dados do ERP pra UMA loja: preço de venda (P{n}, atacado a{n}), custo (custoloja{n}), margens
async function dadosERP(ln, cods, fonte = 'producao') {
  const vazio = { preco: {}, custo: {}, margem: {}, existe: new Set(), desativado: new Set() };
  const qq = fonte === 'teste' ? qTeste : qERP;
  if (!qq) { if (fonte === 'teste') throw new Error('MySQL de teste do .254 não configurado'); return vazio; }
  if (!cods.length) return vazio;
  const ph = cods.map(() => '?').join(',');
  const [itens, custos, margens] = await Promise.all([
    qq(`SELECT CodigoBarra, P${ln} AS P, a${ln} AS A, CodDesativado FROM central.itens WHERE CodigoBarra IN (${ph})`, cods),
    qq(`SELECT CodigoBarra, Custo FROM central.custoloja${ln} WHERE CodigoBarra IN (${ph})`, cods),
    qq(`SELECT CodigoBarra, MargemVarejo, MargemAtacado FROM central.itens_margens WHERE nLoja=? AND CodigoBarra IN (${ph})`, [ln, ...cods])
  ]);
...
// motivo do bloqueio vindo de fora do cálculo, na ordem em que o(a) comprador(a) precisa resolver
...
async function montarEntradas(reg, x) {
  ...
  reg.entradas = base.map(b => {
    const ci = +Number(b.custo_novo).toFixed(4);   // base de preço = custo novo (já com imposto)
...
function editarItem(id, cod, v, usuario) {
  ...
  if (v.preco_final != null) {
    ...
    if (p < it.custo_imposto - 1e-9) return { erro: 'Preço abaixo do custo novo (R$ ' + it.custo_imposto.toFixed(2) + ')' };
  ...
  if (v.preco_atacado_final != null && it.atacado) {
    ...
    if (p < it.custo_imposto - 1e-9) return { erro: 'Preço atacado abaixo do custo novo' };
...
async function setParametros(id, p, usuario) {
  ...
  if (p.politica != null) { if (!calc.POLITICAS.includes(p.politica)) return { erro: 'Regra inválida' }; r.parametros.politica = p.politica; }
  ...
  if (p.ajuste_centavos != null) { const t = String(p.ajuste_centavos).trim(); if (!calc.validaAjuste(t)) return { erro: 'Ajuste de centavos inválido: use "de>para" separado por vírgula, ex.: 89>99,09>19' }; r.parametros.ajuste_centavos = t; }
...
function reabrir(id, usuario) {
  ...
  if (r.erp_teste) { r.erp_teste_anterior = [...(r.erp_teste_anterior || []), r.erp_teste]; delete r.erp_teste; }   // já gravado no ERP teste fica no histórico
...
// Lista fechada → Carga de Itens do Dlinks no MySQL de TESTE (.254): grava P{n}/a{n} em central.itens
// e 1 linha em central.logpreco2 por item, tudo numa transação via escreverERP.lote (Processos > Log).
// Só itens que sobem/descem (e atacado que mudou na L4). Nunca no .252.
async function enviarCarga(id, usuario, escrever) {
  const r = obter(id); if (!r) return null;
  if (r.erp_teste?.logId) return { erro: 'Esta lista já foi enviada pra carga (Log ' + r.erp_teste.logId + '). Reabra e feche de novo pra reenviar.' };
  if (r.status !== 'precificado') return { erro: 'Feche a lista antes de enviar pra Carga de Itens' };
  let montado;
  try { montado = precifErp.montarPassosCarga({ r, usuario }); } catch (e) { return { erro: e.message }; }
  const res = await escrever.lote({ usuario: usuario || 'ECONOMICO', motivo: montado.motivo, banco: 'central', passos: montado.passos, limite: 1 });
  if (!res.ok) return { erro: 'ERP teste recusou: ' + (res.erro || res.status), logId: res.id || null };
  r.status = 'aplicado'; r.aplicadoEm = new Date().toISOString(); r.aplicadoPor = usuario || null;
  r.erp_teste = { logId: res.id, itens: montado.itens.length, cods: montado.itens.map(i => i.cod), em: r.aplicadoEm, por: usuario || null };
  hist(r, 'enviar_carga', usuario, { logId: res.id, itens: montado.itens.length });
  return salvar(r);
}
...
async function verificar(id) {
  ...
  // enviado pra carga no ERP teste → confere lá (o .252 não mudou e marcaria tudo divergente)
  const erp = await dadosERP(r.loja, cods, r.erp_teste?.logId ? 'teste' : 'producao');
  let div = 0;
...
// 5 cenários de teste (um por loja 1..5), sem ERP. Loja 4 tem atacado. Cada chamada de
// criarTestes(usuario, n) cria n registros (default 5), ids T<stamp><k>-L<loja>.
const CENARIOS_TESTE = [
  // [cod, descricao, curvaA, custo_atual, custo_novo, margem, preco_atual, (margem_atacado, preco_atacado_atual, motivo_bloqueio)]
  { loja: 1, pedidoId: 9001, lista_nome: 'TESTE · MASSAS VITARELLA', fornecedor: 'VITARELLA (TESTE)', frete: 30, ipi: 0, st: 0, itens: [
    ['7896213000011', 'MACARRÃO ESPAGUETE 500G (custo subiu → margem)', true, 3.10, 3.45, 28, 3.99],
    ['7896213000028', 'MACARRÃO PARAFUSO 500G (custo igual → margem)', true, 3.20, 3.20, 28, 4.19],
    ['7896213000035', 'BISCOITO CREAM CRACKER 400G (−22% → preço −10%)', false, 3.80, 2.95, 32, 5.29],
    ['7896213000042', 'BISCOITO MARIA 400G (−6%, menos de 10% → mantém)', false, 3.60, 3.40, 32, 4.99],
    ['7896213000059', 'FARINHA DE TRIGO 1KG (subiu e preço atual abaixo do custo)', false, 4.20, 4.60, 20, 4.49]
  ] },
  { loja: 2, pedidoId: 9002, lista_nome: 'TESTE · CAFÉ TRÊS CORAÇÕES', fornecedor: 'TRÊS CORAÇÕES (TESTE)', frete: 45, ipi: 0, st: 80, itens: [
    ['7896005800010', 'CAFÉ EXTRA FORTE 500G (subiu 12% → margem)', true, 14.20, 15.90, 22, 17.99],
    ['7896005800027', 'CAFÉ TRADICIONAL 500G (−20% → preço −10%)', true, 13.10, 10.48, 22, 16.49],
    ['7896005800034', 'CAFÉ SOLÚVEL 200G (sem margem cadastrada)', false, 12.40, 13.10, null, 15.99],
    ['7896005800041', 'CAPPUCCINO 200G (produto novo na loja → margem)', false, null, 9.80, 35, null]
  ] },
  { loja: 3, pedidoId: 9003, lista_nome: 'TESTE · LIMPEZA YPÊ', fornecedor: 'YPÊ (TESTE)', frete: 15, ipi: 6, st: 12, itens: [
    ['7896098900017', 'DETERGENTE 500ML (−22% → preço −10%)', true, 2.05, 1.60, 40, 2.89],
    ['7896098900024', 'AMACIANTE 2L (−11%, mais de 10% → preço −10%)', true, 8.90, 7.95, 38, 12.49],
    ['7896098900031', 'SABÃO EM PÓ 1KG (custo igual → margem)', false, 9.10, 9.10, 30, 11.99],
    ['7896098900048', 'DESINFETANTE 2L (subiu 2 centavos → margem)', false, 6.20, 6.22, 42, 8.79],
    ['7896098900055', 'ESPONJA 3UN (subiu → margem)', false, 2.30, 2.75, 55, 3.59]
  ] },
  { loja: 4, pedidoId: 9004, lista_nome: 'TESTE · ARROZ E FEIJÃO', fornecedor: 'FORNECEDOR TESTE', frete: 20, ipi: 0, st: 28, atacado: true, itens: [
    ['7891000100103', 'ARROZ TIPO 1 5KG (subiu → margem, varejo e atacado)', true, 22.90, 24.50, 18, 27.99, 12, 26.49],
    ['7891000100110', 'FEIJÃO CARIOCA 1KG (−23% → preço −10% no varejo e no atacado)', true, 7.80, 6.00, 25, 9.99, 15, 9.29],
    ['7891000100127', 'BISCOITO 400G (−12%, mais de 10% → preço −10% no varejo e no atacado)', true, 4.10, 3.60, 35, 5.69, 20, 5.19],
    ['7891000100134', 'DETERGENTE 500ML (custo igual → margem)', false, 1.90, 1.90, 40, 2.69, null, null],
    ['7891000100141', 'SABÃO EM PÓ 1KG (sem margem cadastrada)', false, 8.20, 8.90, null, 11.49, null, null],
    ['7891000100158', 'AZEITE 500ML (produto novo na loja → margem)', false, null, 24.00, 30, null, null, null]
  ] },
  { loja: 5, pedidoId: 9005, lista_nome: 'TESTE · BEBIDAS AMBEV', fornecedor: 'AMBEV (TESTE)', semRateio: true, itens: [
    ['7891991000017', 'CERVEJA LATA 350ML (subiu → margem)', true, 2.85, 3.05, 25, 3.69],
    ['7891991000024', 'REFRIGERANTE 2L (−22% → preço −10%)', true, 5.40, 4.20, 30, 7.29],
    ['7891991000031', 'ÁGUA 500ML (custo igual → margem)', false, 0.95, 0.95, 60, 1.59],
    ['7891991000048', 'ENERGÉTICO 250ML (não casado no ERP)', false, 4.10, 4.30, 40, 6.49, null, null, 'não casado no ERP (código 7891991000048)'],
    ['7891991000055', 'SUCO 1L (unidade não convertida)', false, 5.60, 6.10, 30, 7.99, null, null, 'unidade da nota (caixa/fardo) não convertida — conferir']
  ] }
];
function criarTestes(usuario, n = 5) {
  const stamp = Date.now().toString().slice(-6);
  const out = [];
  for (let k = 0; k < Math.min(n, CENARIOS_TESTE.length); k++) {
    const c = CENARIOS_TESTE[k];
    // imposto da nota fica só informativo: base de preço = custo novo (já com imposto)
    const fator = 0;
    const entradas = c.itens.map(([cod, descricao, curvaA, ca, cn, m, pa, ma, paa, bloq]) => ({
      cod, descricao, curvaA, recebida: 12 + k * 6, nao_pedido: false, custo_atual: ca, custo_novo: cn, custo_imposto: +Number(cn).toFixed(4),
      margem: m, preco_atual: pa, margem_atacado: c.atacado ? (ma ?? null) : null, preco_atacado_atual: c.atacado ? (paa ?? null) : null,
      ...(bloq ? { motivo_bloqueio: bloq } : {})
    }));
    const valorProduto = +entradas.reduce((s, e) => s + e.recebida * e.custo_novo, 0).toFixed(2);
    const reg = { id: `T${stamp}${k}-L${c.loja}`, pedidoId: c.pedidoId, loja: c.loja, lista: 0, lista_nome: c.lista_nome, fornecedor: c.fornecedor, teste: true,
      status: 'a_precificar', criadoEm: new Date().toISOString(), conciliadoEm: new Date(Date.now() - k * 3600e3).toISOString(), curva_disponivel: true,
      parametros: { ...padrao },
      rateio: c.semRateio ? { frete: 0, ipi: 0, st: 0, desconto: 0, valorProduto: 0, fator: 0, disponivel: false }
                          : { frete: c.frete, ipi: c.ipi || 0, st: c.st, desconto: 0, valorProduto, fator, disponivel: true },
      entradas, itens: [], resumo: null, historico: [{ acao: 'criado', por: usuario || null, em: new Date().toISOString(), teste: true }] };
    calc.calcularRegistro(reg);
    out.push(salvar(reg));
  }
  return out;
}
function criarTeste(usuario) { return criarTestes(usuario, 1)[0]; }
...
function gerarPdf(r) {
  ...
  doc.fontSize(8.5).fillColor('#C9D1E3').text(`Pedido #${r.pedidoId} · ${r.lista_nome} · ${r.fornecedor} · conciliado ${dt(r.conciliadoEm)} · regra: custo ≥ atual → margem; mais de 10% mais barato → preço −10% · terminação ${r.parametros.arredondamento}`, 40, 76, { width: W - 24 });
  let y = 104;
  const l4 = r.loja === 4;
...
module.exports = { init, initERP, setPadrao, getPadrao, criarDeConciliacao, criarTeste, criarTestes, recalcular, listar, obter, salvar, idDe, itensConciliados, rateioDe, editarItem, setParametros, fechar, reabrir, aplicar, enviarCarga, verificar, verificarTodos, removerTestes, gerarPdf, caminhoPdf, LOJAS_NOMES };
