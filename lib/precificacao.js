...
const CENARIOS_TESTE = [
  { loja: 1, pedidoId: 9001, lista_nome: 'TESTE · MASSAS VITARELLA', fornecedor: 'VITARELLA (TESTE)', fator: 0.025, frete: 30, st: 0, itens: [
  ...
  { loja: 2, pedidoId: 9002, lista_nome: 'TESTE · CAFÉ TRÊS CORAÇÕES', fornecedor: 'TRÊS CORAÇÕES (TESTE)', fator: 0.061, frete: 45, st: 80, itens: [
  ...
  { loja: 3, pedidoId: 9003, lista_nome: 'TESTE · LIMPEZA YPÊ', fornecedor: 'YPÊ (TESTE)', fator: 0.018, frete: 15, st: 12, itens: [
  ...
  { loja: 4, pedidoId: 9004, lista_nome: 'TESTE · ARROZ E FEIJÃO', fornecedor: 'FORNECEDOR TESTE', fator: 0.04, frete: 20, st: 28, atacado: true, itens: [
...
function criarTestes(usuario, n = 5) {
  ...
  for (let k = 0; k < Math.min(n, CENARIOS_TESTE.length); k++) {
    ...
    const fator = c.semRateio ? 0 : c.fator;
    ...
    const reg = { id: `T${stamp}${k}-L${c.loja}`, pedidoId: c.pedidoId, loja: c.loja, lista: 0, lista_nome: c.lista_nome, fornecedor: c.fornecedor, teste: true,
      ...
      rateio: c.semRateio ? { frete: 0, ipi: 0, st: 0, desconto: 0, valorProduto: 0, fator: 0, disponivel: false }
                          : { frete: c.frete, ipi: 0, st: c.st, desconto: 0, valorProduto, fator, disponivel: true },
