...
function rateioDe(x) {
  ...
  // só IMPOSTO entra no custo (IPI + ST). Frete e desconto ficam guardados pra informação, mas não
  // mexem no preço (decisão do Tiago, 11/09/2026).
  const fator = disponivel ? +(((s.ipi + s.st) / s.valorProduto)).toFixed(6) : 0;
...
function criarTestes(usuario, n = 5) {
  ...
  for (let k = 0; k < Math.min(n, CENARIOS_TESTE.length); k++) {
    ...
    // fator vem de IPI+ST sobre o valor dos produtos (frete não entra)
    const valorProdutoPrev = +c.itens.reduce((s, it, j) => s + (12 + k * 6) * it[4], 0).toFixed(2);
    const fator = c.semRateio ? 0 : +(((c.ipi || 0) + (c.st || 0)) / valorProdutoPrev).toFixed(6);
    const entradas = c.itens.map(([cod, descricao, curvaA, ca, cn, m, pa, ma, paa, bloq]) => ({
      cod, descricao, curvaA, recebida: 12 + k * 6, nao_pedido: false, custo_atual: ca, custo_novo: cn, custo_imposto: +(cn * (1 + fator)).toFixed(4),
