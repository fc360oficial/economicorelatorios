...
// Itens App traz o PESO como embalagem em saco de ração ("RACAO ... SC20KG" com Qtd_venda 20): é quilo, não unidades por
// caixa. Vende-se por saco, então un/cx = 1 (Tiago, 16/09/2026). Vale quando o número da embalagem bate com o KG do nome.
function embalagemEhPeso(descricao, qtdEmb) {
  const m = String(descricao || '').toUpperCase().match(/(?:SC|SACO|PCT|PC)\s?(\d+(?:[.,]\d+)?)\s?KG/);
  if (!m) return false;
  return Math.abs(parseFloat(m[1].replace(',', '.')) - (+qtdEmb)) < 0.01;
}
// Caixas a partir do valor da linha da nota (o CD fatura ao custo por caixa do app). Se não há custo, ou o valor
// não fecha num inteiro (±3 % — preço mudou), volta pra quantidade: unidades ÷ un/cx.
function caixasPorValor(total, custoCx, unidades, unPorCaixa) {
  if (total > 0 && custoCx > 0) {
    const cx = total / custoCx; const r = Math.round(cx);
    if (r >= 1 && Math.abs(cx - r) <= 0.03 * r) return r;
  }
  return +((unidades || 0) / (unPorCaixa || 1)).toFixed(2);
}
module.exports = { caixasPorValor, ean13Valido, dun14ParaEan13, emCaixas, distribuirCdInsuficiente, statusRecebimento, mediaLead, tokensDescricao, casarPorDescricao, mediaMensal, embalagemEhPeso };
// Venda/dia de um produto NOVO: divide pelos dias desde a primeira venda (piso 7 d), não pela janela inteira —
// senão 24 un vendidas em 5 dias viram 0,6/dia (÷40) e a sugestão fica 1 cx por semana enquanto a loja zera.
// Conforme os dias passam o divisor cresce até a janela e a regra volta ao normal sozinha. Produto que já vendia
// em meses anteriores (voltou de falta) não é novo: usa a janela. Datas em 'YYYY-MM-DD'; dFim é o último dia da janela.
function vendaDiaNova(qtd, primeiraVenda, dFim, janela, jaVendiaAntes) {
  const normal = { vq: +((qtd || 0) / janela).toFixed(3), dias: janela, nova: false };
  if (!qtd || !primeiraVenda || jaVendiaAntes) return normal;
  const dias = Math.round((Date.parse(dFim + 'T00:00:00Z') - Date.parse(primeiraVenda + 'T00:00:00Z')) / 86400000) + 1;
  if (dias >= janela || dias < 1) return normal;
  const div = Math.max(7, dias);
  return { vq: +(qtd / div).toFixed(3), dias: div, nova: true };
}
module.exports.vendaDiaNova = vendaDiaNova;
