...
// Compradora encerra a digitação do vendedor (= "Fechar Solicitar Preço Web" do Dlinks)
function fechar(id, usuario) {
  ...
  return finalizar(p.token, (usuario ? usuario + ' ' : '') + '(compradora)');
}
// "Gerar com Custo" (Dlinks: MultLoja=1): a compradora fecha o pedido na hora com o custo como preço, sem link do vendedor.
...
function fecharComCusto(id, precos, usuario) {
  ...
  return finalizar(p.token, (usuario ? usuario + ' ' : '') + '(compradora, com custo)');
}
// Compradora reabre a digitação (= "Abrir Solicitar Preço Web" do Dlinks): o link volta a aceitar preços
...
function observacoesPedido(p) {
  ...
  if (p.obs_fiscal) out.push({ chave: 'compradora', rotulo: 'Observação da compradora (Fiscal e fornecedor)', texto: p.obs_fiscal, por: p.obs_fiscal_por || null });
...
// O vendedor pode responder (campo de texto); a resposta aparece pra compradora e vai no PDF.
