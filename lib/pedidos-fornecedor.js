...
//   1. comprador(a) seleciona listas no Radar e clica "Enviar pro fornecedor"
...
//   5. no pedido finalizado o(a) comprador(a) vê último custo × preço digitado com
...
function criar({ lista, cadastro, detalhe, teto, embMeses, usuario, modo, origem }) {
  ...
  const p = {
    ...
    abertoEm: null, finalizadoEm: null, parametros: { teto, embMeses, fazer_em: detalhe.fazer_em, gatilho: detalhe.gatilho || 'lista', fazer_em_lista: detalhe.fazer_em_lista ?? detalhe.fazer_em, modo: modo || 'completa', origem: origem || 'radar' },
...
function salvarPrecos(token, itens, avariaResposta, obsPedido) {
  const p = porToken(token); if (!p) return null;
  if (typeof obsPedido === 'string') { p.obs_vendedor = obsPedido.slice(0, 600); p.obs_vendedor_em = new Date().toISOString(); }
  if (typeof avariaResposta === 'string' && p.avarias) { p.avarias.resposta_vendedor = avariaResposta.slice(0, 500); p.avarias.resposta_em = new Date().toISOString(); }
  ...
  for (const it of p.itens) {
    ...
    if (typeof n.emb_vendedor === 'string') it.emb_vendedor = n.emb_vendedor.trim().slice(0, 20);   // embalagem informada pelo vendedor (só observação)
  }
...
// Comprador(a) encerra a digitação do vendedor (= "Fechar Solicitar Preço Web" do Dlinks)
function fechar(id, usuario) {
  const p = obter(id); if (!p) return null;
  if (!['aguardando', 'digitacao'].includes(p.status)) return { erro: 'Só pedido aguardando/em digitação pode ser fechado (status atual: ' + p.status + ')' };
  return finalizar(p.token, (usuario ? usuario + ' ' : '') + '(comprador(a))');
}
// "Gerar com Custo" (Dlinks: MultLoja=1): o(a) comprador(a) fecha o pedido na hora com o custo como preço, sem link do vendedor.
// precos = { cod: preco } vindo da sugestão (preco_und); item sem preço informado usa o último custo.
function fecharComCusto(id, precos, usuario) {
  const p = obter(id); if (!p) return null;
  if (!['aguardando', 'digitacao'].includes(p.status)) return { erro: 'Só pedido aguardando/em digitação pode ser fechado com custo (status atual: ' + p.status + ')' };
  const m = precos && typeof precos === 'object' ? precos : {};
  for (const it of p.itens) {
    const v = parseFloat(String(m[it.cod] ?? m[String(it.cod)] ?? '').replace(',', '.'));
    it.preco = isFinite(v) && v > 0 ? +v.toFixed(4) : (it.ultimo_custo > 0 ? +(+it.ultimo_custo).toFixed(4) : null);
  }
  p.com_custo = true; p.atualizadoEm = new Date().toISOString(); salvar(p);   // finalizar relê do disco pelo token
  return finalizar(p.token, (usuario ? usuario + ' ' : '') + '(comprador(a), com custo)');
}
// Comprador(a) reabre a digitação (= "Abrir Solicitar Preço Web" do Dlinks): o link volta a aceitar preços
function reabrir(id, usuario) {
  const p = obter(id); if (!p) return null;
  if (p.status !== 'finalizado') return { erro: 'Só pedido finalizado pode ser reaberto (status atual: ' + p.status + ')' };
  p.status = 'digitacao'; p.reabertoEm = new Date().toISOString(); p.reabertoPor = usuario || null; p.finalizadoEm = null; p.finalizadoPor = null;
  return salvar(p);
}
// --- lado do(a) comprador(a) ---
...
function observacoesPedido(p) {
  const out = [];
  if (p.avarias && (p.avarias.itens || []).length) out.push({ chave: 'avarias', rotulo: 'Resposta do vendedor sobre as avarias', texto: p.avarias.resposta_vendedor || '', vazio: !p.avarias.resposta_vendedor, vazioTexto: 'ainda sem resposta' });
  if (p.obs_fiscal) out.push({ chave: 'compradora', rotulo: 'Observação do(a) comprador(a) (Fiscal e fornecedor)', texto: p.obs_fiscal, por: p.obs_fiscal_por || null });
  if (p.obs_vendedor) out.push({ chave: 'vendedor', rotulo: 'Observação do vendedor sobre o pedido', texto: p.obs_vendedor });
  return out;
}
function gerarPdf(p, soLoja) {
  ...
  let y = 112;
  const obsL = observacoesPedido(p);
  if (obsL.length) {
    doc.font('Helvetica').fontSize(8.5);
    const linhas = obsL.map(o => o.rotulo + ': ' + (o.vazio ? o.vazioTexto : o.texto));
    const alt = 18 + linhas.reduce((s, t) => s + doc.heightOfString(t, { width: W - 16 }) + 3, 0);
    doc.roundedRect(28, y, W, alt, 4).lineWidth(0.6).fillAndStroke('#FFF6D9', '#EFD98F');
    doc.fillColor('#6B4E00').font('Helvetica-Bold').fontSize(7).text('OBSERVAÇÕES', 36, y + 5);
    let yy = y + 15;
    for (const o of obsL) { doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#0E1626').text(o.rotulo + ': ', 36, yy, { continued: true, width: W - 16 }).font('Helvetica').fillColor(o.vazio ? '#C22F49' : '#0E1626').text(o.vazio ? o.vazioTexto : o.texto, { width: W - 16 }); yy += doc.heightOfString(o.rotulo + ': ' + (o.vazio ? o.vazioTexto : o.texto), { width: W - 16 }) + 3; }
    y += alt + 8;
  }
  // resumo por loja
  ...
  if (avs.length) {
    ...
    doc.fillColor('#C22F49').font('Helvetica-Bold').fontSize(9.5).text(`AVARIAS PENDENTES COM O FORNECEDOR · ${avs.length} item(ns) · ${brl(avs.reduce((s, a) => s + a.total, 0))} · pedido autorizado condicionado à resolução`, 36, y + 5);
    y += 24;
    if (p.avarias?.resposta_vendedor) { doc.font('Helvetica').fontSize(8.5).fillColor('#0E1626').text('Resposta do vendedor: ' + p.avarias.resposta_vendedor, 36, y, { width: W }); y += 18; }
...
function ajustarEmb(id, cod, emb, usuario) {
  const p = obter(id); if (!p) return null;
  if (['aprovado', 'recebido', 'recebido_parcial', 'cancelado'].includes(p.status)) return { erro: 'Pedido ' + p.status + ': a caixa padrão do produto foi gravada, mas este pedido não muda mais' };
  const it = p.itens.find(i => String(i.cod) === String(cod)); if (!it) return { erro: 'Item não está no pedido' };
  const e = Math.round(+emb); if (!(e >= 1)) return { erro: 'Embalagem inválida' };
  it.emb = e; it.volumes = it.qtd ? Math.ceil(it.qtd / e) : 0; it.emb_ajustada_por = usuario || null;
  p.totais = totais(p); return salvar(p);
}
function obsFiscal(id, texto, usuario) {
  const p = obter(id); if (!p) return null;
  if (p.status === 'cancelado') return { erro: 'Pedido cancelado' };
  p.obs_fiscal = String(texto || '').trim().slice(0, 600) || null; p.obs_fiscal_por = usuario || null; p.obs_fiscal_em = new Date().toISOString();
  if (p.pdf) { try { gerarPdf(p); } catch (e) { console.error('[PEDIDOS] pdf obs fiscal:', e.message); } }   // PDF aprovado reflete a observação
  return salvar(p);
}
function excluir(id, usuario) {
  const p = obter(id); if (!p) return null;
  if (['aprovado', 'recebido', 'recebido_parcial'].includes(p.status)) return { erro: 'Pedido ' + p.status + ' não pode ser excluído (cancele ou feche pelo fluxo normal)' };
  const dirEx = path.join(DIR, 'excluidos'); fs.mkdirSync(dirEx, { recursive: true });
  p.excluidoEm = new Date().toISOString(); p.excluidoPor = usuario || null;
  fs.writeFileSync(path.join(dirEx, p.id + '.json'), JSON.stringify(p));
  try { fs.unlinkSync(arq(p.id)); } catch (e) {}
...
// comprador(a) ajusta quantidades por loja dentro do pedido (antes de aprovar)
...
// sugestão criada pelo sistema (ruptura de entrega) → comprador(a) manda pro vendedor
...
// status "sugestao" até o(a) comprador(a) mandar pro vendedor.
// ─────────────────────────────────────────────────────────────
...
async function verificarRecebimentos() {
  const r = await cx.conferirTodos({ listar, salvar, criarOuMesclarRuptura, onConciliado: hooks.onConciliado });
  // depois do XML, vê quais notas já deram ENTRADA na loja (central.compras) — 24/09/2026
  try { r.entradas = await cx.verificarEntradas({ listar, salvar }); } catch (e) { console.error('[PEDIDOS] entradas:', e.message); }
  return r;
}
...
function removerTestesXml() {
  let n = 0;
  // também os pedidos nascidos de cotação de TESTE (Cotação › Criar exemplos): p.teste, fornecedor "FORNECEDOR TESTE 1/2 (...)" ou cotação marcada como teste
  const cotTeste = id => { try { const c = require('./cotacao').obter(id); return !!(c && c.teste); } catch (e) { return false; } };
  const ehTeste = p => p.teste || /^FORNECEDOR TESTE/i.test(p.fornecedor || '') || (p.cotacao?.id != null && cotTeste(p.cotacao.id)) || (p.origem?.pedidoId && (obter(p.origem.pedidoId) ? ehTeste(obter(p.origem.pedidoId)) : false));
  for (const p of listar()) if (ehTeste(p)) {
...
// Comprador(a) aceita a consistência de UMA loja e fecha (motivo obrigatório). A loja vira "conciliado"
...
// Decisão do(a) comprador(a) ANTES do caminhão chegar (o XML entra no ERP antes da entrega):
...
function recusasLoja(p, ln) {
  ...
  for (const i of x.itens || []) {
    ...
    else out.push({ cod: i.cod, descricao: i.descricao, qtd: i.recebida, motivo: 'recusado pelo(a) comprador(a)', preco: i.preco_xml });
  }
...
// Coletor de Recebimento (25/09/2026): o mesmo aviso, mas a partir da conferência cega da loja
// (lib/recebimento.js) em vez do pedido — as linhas vêm com origem (compras / coletor / falta).
const DIR_RECEB = path.join(__dirname, '..', 'data', 'recebimento');
const DEV_ORIGEM = { compras: 'Compras', coletor: 'Coletor', falta: 'Falta' };
// devolve uma Promise que resolve no 'finish' do arquivo: quem chama só manda o PDF depois de escrito
function gerarPdfDevolucaoColetor(extras, usuario) {
  const PDFDocument = require('pdfkit');
  const ln = +extras.loja || 0; const nota = extras.nota || {};
  const itens = [...(extras.itens || [])].sort((a, b) => String(a.origem).localeCompare(String(b.origem)) || String(a.descricao).localeCompare(String(b.descricao)));
  const dir = extras.dir || DIR_RECEB;   // extras.dir só é usado pelos testes
  fs.mkdirSync(dir, { recursive: true });
  // o nome vem do id da conferência, que carrega o nº da nota digitado no celular: sanitiza
  const nome = String(extras.id || nota.nNota || 'nota').replace(/[^\w.-]/g, '_').slice(0, 80);
  const destino = path.join(dir, `${nome}-devolucao.pdf`);
  const doc = new PDFDocument({ size: 'A4', margin: 36, info: { Title: `Aviso de devolução - NF-e ${nota.nNota || ''} - Loja ${ln}` } });
  const saida = fs.createWriteStream(destino);
  const pronto = new Promise((resolve, reject) => { saida.on('finish', () => resolve(destino)); saida.on('error', reject); doc.on('error', reject); });
  doc.pipe(saida);
  ...
  doc.fillColor('#FFC933').font('Helvetica-Bold').fontSize(9).text('AVISO DE DEVOLUÇÃO NO RECEBIMENTO · ECONOMICO SUPERMERCADO - REDE CAHU', 80, 50);
  doc.fillColor('#FFFFFF').fontSize(15).text(`NF-e ${nota.nNota || '—'} · Loja ${ln} · ${LOJAS_NOMES[ln] || ''}`, 48, 74);
  doc.font('Helvetica').fontSize(8.5).fillColor('#AEB8CE').text(`Fornecedor: ${nota.fornecedor || '—'}${nota.chave ? '  ·  Chave ' + nota.chave : ''}  ·  Emitido em ${new Date().toLocaleString('pt-BR')}${usuario ? ' por ' + usuario : ''}`, 48, 94, { width: W - 24 });
  let y = 128;
  doc.rect(36, y, W, 22).fill('#FBEAED');
  doc.fillColor('#C22F49').font('Helvetica-Bold').fontSize(10).text(`ITENS A DEVOLVER · ${itens.length} item(ns) · conferência física da loja (contagem cega)`, 44, y + 6); y += 30;
  const cols = [['Origem', 58], ['Código', 78], ['Produto', W - 58 - 78 - 52 - 165], ['Qtd', 52], ['Motivo', 165]];
  ...
  for (const [k, r] of itens.entries()) {
    const motivo = r.origem === 'falta' ? 'não veio — o motorista assina reconhecendo' : (r.motivo || '');
    // produto e motivo quebram linha (nome de produto do ERP é longo); a linha cresce junto
    doc.font('Helvetica').fontSize(8.5);
    const alt = Math.max(16, doc.heightOfString(String(r.descricao || ''), { width: cols[2][1] - 6 }) + 3, doc.heightOfString(motivo, { width: 159 }) + 3);
    if (y + alt > doc.page.height - 90) { doc.addPage(); y = 40; }
    if (k % 2) doc.rect(36, y - 3, W, alt).fill('#FAFAF8');
    xx = 36; doc.font('Helvetica').fontSize(8.5).fillColor('#0E1626');
    for (const [val, w, quebra] of [[DEV_ORIGEM[r.origem] || r.origem, 58, false], [r.cod, 78, false], [r.descricao, cols[2][1], true], [n3(r.qtd) + ' un', 52, false], [motivo, 165, true]]) {
      doc.text(String(val == null ? '' : val), xx + 3, y, { width: w - 6, lineBreak: quebra, height: quebra ? alt : undefined }); xx += w;
    }
    y += alt;
  }
  if (!itens.length) { doc.font('Helvetica').fontSize(9).fillColor('#4E5A72').text('Nenhum item de devolução nesta nota.', 39, y); y += 18; }
  y += 14;
  doc.font('Helvetica').fontSize(8.5).fillColor('#4E5A72').text('Origem da devolução: COMPRAS = recusado pelo(a) comprador(a) na conferência do XML · COLETOR = validade curta ou avaria vista na conferência física · FALTA = item que está na nota e não veio; o motorista assina reconhecendo a falta. Documento gerado pelo Econômico Relatórios a partir da conferência cega da loja.', 36, y, { width: W }); y += 46;
  doc.font('Helvetica-Bold').fontSize(9).fillColor('#0E1626').text('Motorista / entregador', 36, y); doc.font('Helvetica').fontSize(8.5).fillColor('#4E5A72').text('Nome e assinatura', 36, y + 13);
  doc.moveTo(36, y + 34).lineTo(36 + W / 2 - 14, y + 34).lineWidth(0.7).strokeColor('#0E1626').stroke();
  doc.font('Helvetica-Bold').fontSize(9).fillColor('#0E1626').text('Conferente / fiscal', 36 + W / 2 + 14, y); doc.font('Helvetica').fontSize(8.5).fillColor('#4E5A72').text('Nome e assinatura', 36 + W / 2 + 14, y + 13);
  doc.moveTo(36 + W / 2 + 14, y + 34).lineTo(36 + W, y + 34).stroke();
...
// Com `extras` ({ id, itens: devolucoes[], nota, loja, motorista }) a lista vem do Coletor de
// Recebimento em vez de recusasLoja(p, ln) e o retorno é uma Promise com o caminho (resolvida
// quando o arquivo termina de ser escrito) — o caminho antigo (sem extras) continua igual.
function gerarPdfDevolucao(p, ln, usuario, extras) {
  if (extras && Array.isArray(extras.itens)) return gerarPdfDevolucaoColetor(extras, usuario);
  const PDFDocument = require('pdfkit');
...
// As avarias ENTRAM NO PEDIDO como itens de reposição (igual à ruptura): uma linha por produto, na loja da
// avaria, com a quantidade da avaria, marcada origem_item='avaria'. Reconsultar refaz do zero (tira e põe).
function incorporarAvarias(p) {
  // 1) remove o que foi incorporado antes
  for (const it of p.itens) {
    if (!it.avaria) continue;
    for (const [l, a] of Object.entries(it.avaria)) { it.lojas_qtd[l] = Math.max(0, (it.lojas_qtd[l] || 0) - a.qtd_pedido); if (!it.lojas_qtd[l]) delete it.lojas_qtd[l]; }
    delete it.avaria; if (it.origem_item === 'avaria') it.origem_item = 'lista';
  }
  p.itens = p.itens.filter(it => Object.values(it.lojas_qtd).some(q => q > 0));
  // 2) incorpora as atuais
  for (const a of p.avarias?.itens || []) {
    if (!(a.qtd > 0)) continue;
    let it = p.itens.find(i => i.cod === a.cod);
    if (!it) { it = { cod: a.cod, descricao: a.descricao, unid: a.und || 'UN', emb: 1, qtd: 0, volumes: 0, lojas_qtd: {}, ultimo_custo: a.valor || 0, preco: null, obs: '', origem_item: 'avaria', avaria: {} }; p.itens.push(it); }
    it.avaria = it.avaria || {};
    const q = /KG|G|L|ML/.test(String(a.und || '').toUpperCase()) ? +a.qtd.toFixed(3) : Math.ceil(a.qtd);
    it.lojas_qtd[a.loja] = +((it.lojas_qtd[a.loja] || 0) + q).toFixed(3);
    const prev = it.avaria[a.loja] || { qtd_pedido: 0, qtd: 0, total: 0, itens: [] };
    it.avaria[a.loja] = { qtd_pedido: +(prev.qtd_pedido + q).toFixed(3), qtd: +(prev.qtd + a.qtd).toFixed(3), total: +(prev.total + a.total).toFixed(2), status: a.status, data: a.data, itens: [...prev.itens, a.nReg] };
  }
  // item que só existe por causa da avaria → origem 'avaria'; item da lista que ganhou reposição mantém a origem
  for (const it of p.itens) if (it.avaria && Object.keys(it.lojas_qtd).every(l => it.avaria[l] && it.avaria[l].qtd_pedido >= (it.lojas_qtd[l] || 0))) it.origem_item = 'avaria';
  for (const it of p.itens) { it.qtd = +Object.values(it.lojas_qtd).reduce((s, v) => s + (v || 0), 0).toFixed(3); it.volumes = it.qtd ? Math.ceil(it.qtd / (it.emb || 1)) : 0; }
  p.lojas = [...new Set(p.itens.flatMap(i => Object.keys(i.lojas_qtd).filter(l => i.lojas_qtd[l] > 0).map(Number)))].sort((a, b) => a - b);
  p.totais = totais(p);
  p.resumo_origem = { ...(p.resumo_origem || {}), avaria: p.itens.filter(i => i.avaria).length };
  return p;
}
// Decisão do Tiago (11/09/2026, noite): avaria NÃO vira item do pedido. Fica um BLOCO separado que o vendedor vê
// embaixo dos itens que ele precifica, com o aviso "o pedido só é autorizado com a resolução das avarias".
// O vendedor pode responder (campo de texto); a resposta aparece pra comprador(a) e vai no PDF.
async function anexarAvarias(p) {
  // Regra do Tiago (25/09/2026): pedido nascido de COTAÇÃO não traz avarias (nem na criação, nem no Reconsultar).
  if (p.parametros?.origem === 'cotacao') { if (p.avarias) delete p.avarias; return salvar(p); }
  if (p.itens.some(i => i.avaria)) incorporarAvarias.desfazer(p);   // limpa incorporação antiga, se houver
  if (p.teste) { if (p.avarias?.itens) p.avarias = { ...p.avarias, ...resumoAvarias(p.avarias.itens), itens: p.avarias.itens }; return salvar(p); }
  const resposta = p.avarias?.resposta_vendedor || null;
  // Regra do Tiago (11/09/2026): avaria é POR LOJA e POR PRODUTO DA LISTA naquela loja.
  // Só entram avarias das lojas do pedido, de produtos que estão na lista de compra habilitados pra aquela loja
  // (c_cotacao_lista_itens.l1..l6). Avaria de outro produto/loja do mesmo fornecedor fica de fora.
  const todas = await buscarAvarias(p.codFornec);
  let permitido = null;
  if (p.lista && qERP) {
    try {
      const li = await qERP(`SELECT Codigobarra cod, l1, l2, l3, l4, l5, l6 FROM central.c_cotacao_lista_itens WHERE nCotacao=?`, [p.lista]);
      permitido = {}; for (const r of li) permitido[String(r.cod)] = [1, 2, 3, 4, 5, 6].filter(l => r['l' + l]);
    } catch (e) { console.error('[PEDIDOS] itens da lista p/ avarias:', e.message); }
  }
  const lojasPedido = new Set((p.lojas || []).map(Number));
  const itens = todas.filter(a => lojasPedido.has(+a.loja) && (!permitido || (permitido[a.cod] || []).includes(+a.loja)));
  p.avarias = { consultadoEm: new Date().toISOString(), ...resumoAvarias(itens), itens, resposta_vendedor: resposta, fora_do_filtro: todas.length - itens.length };
  return salvar(p);
}
incorporarAvarias.desfazer = function (p) {
  for (const it of p.itens) {
    if (!it.avaria) continue;
    for (const [l, a] of Object.entries(it.avaria)) { it.lojas_qtd[l] = Math.max(0, (it.lojas_qtd[l] || 0) - a.qtd_pedido); if (!it.lojas_qtd[l]) delete it.lojas_qtd[l]; }
    delete it.avaria; if (it.origem_item === 'avaria') it.origem_item = 'lista';
  ...
  for (const it of p.itens) { it.qtd = +Object.values(it.lojas_qtd).reduce((s, v) => s + (v || 0), 0).toFixed(3); it.volumes = it.qtd ? Math.ceil(it.qtd / (it.emb || 1)) : 0; }
  p.lojas = [...new Set(p.itens.flatMap(i => Object.keys(i.lojas_qtd).filter(l => i.lojas_qtd[l] > 0).map(Number)))].sort((a, b) => a - b);
  p.totais = totais(p); if (p.resumo_origem) delete p.resumo_origem.avaria;
};
...
function textoAvarias(p, ln) {
  ...
  const L = ['AVARIAS PENDENTES com o fornecedor (' + its.length + (its.length > 1 ? ' itens' : ' item') + ' · R$ ' + Number(its.reduce((s, a) => s + a.total, 0)).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) + '). O pedido só será autorizado com a resolução destas avarias. Elas aparecem no link do pedido, embaixo dos itens.'];
...
// Cotação (lib/cotacao.js): pedido gerado ao fechar uma cotação guarda de onde veio (badge e link em Pedidos de Compra)
function marcarTeste(id) { const p = obter(id); if (!p) return null; p.teste = true; return salvar(p); }
function vincularCotacao(id, cot) {
  const p = obter(id); if (!p) return null;
  p.cotacao = { id: cot.id, nome: cot.nome || null, entrega: cot.entrega || null, tipo_entrega: cot.tipo_entrega || null, obs: cot.obs || null };
  return salvar(p);
}
...
function visaoVendedor(p) {
  return {
    id: p.id, status: p.status, lista_nome: p.lista_nome, fornecedor: p.fornecedor, criadoEm: p.criadoEm, finalizadoEm: p.finalizadoEm, aprovadoEm: p.aprovadoEm || null, cotacao: p.cotacao ? { id: p.cotacao.id, nome: p.cotacao.nome } : null,
    vendedor: p.vendedor ? { nome: p.vendedor.nome || null } : null,
    comprador: p.comprador, prazo_pagamento: p.prazo_pagamento, lojas: p.lojas, lojas_nomes: LOJAS_NOMES, obs_vendedor: p.obs_vendedor || '',
    itens: p.itens.map(i => ({ cod: i.cod, descricao: i.descricao, unid: i.unid, emb: i.emb, qtd: i.qtd, volumes: i.volumes, lojas_qtd: i.lojas_qtd, preco: i.preco, obs: i.obs, emb_vendedor: i.emb_vendedor || '' })),
    avarias: p.avarias && p.avarias.itens?.length ? { n: p.avarias.n, total: p.avarias.total, resposta_vendedor: p.avarias.resposta_vendedor || '', itens: p.avarias.itens.map(a => ({ loja: a.loja, cod: a.cod, descricao: a.descricao, qtd: a.qtd, und: a.und, valor: a.valor, total: a.total, data: a.data, status: a.status })) } : null
  };
}

module.exports = { salvar, excluir, obsFiscal, ajustarEmb, observacoesPedido, init, initERP, criar, listar, obter, porToken, abrir, salvarPrecos, finalizar, aprovar, cancelar, enviar, fechar, fecharComCusto, reabrir, ajustarQuantidades, verificarRecebimentos, criarTestesXml, removerTestesXml, verAlertas, aceitarLojaXml, financeiroAlertas, decidirItemXml, recusasLoja, gerarPdfDevolucao, anexarAvarias, buscarAvarias, textoAvarias, criarTestesAvaria, visaoVendedor, gerarPdf, caminhoPdf, porLoja, totaisLoja, LOJAS_NOMES, setHooks, vincularCotacao, marcarTeste };
