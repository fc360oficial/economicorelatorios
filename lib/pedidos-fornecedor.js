...
function criar({ lista, cadastro, detalhe, teto, embMeses, usuario, modo }) {
  ...
  const itens = detalhe.itens.filter(i => i.qtd > 0).map(i => ({
    ...
    origem_item: i.risco_a ? 'curva_a' : 'lista', curva_a: !!i.curva_a, cobertura_dias: i.cobertura_dias ?? null
  }));
  const p = {
...
function gerarPdf(p) {
  ...
  doc.fillColor('#FFC933').font('Helvetica-Bold').fontSize(9).text('PEDIDO DE COMPRA APROVADO · ECONÔMICO', 40, 36);
