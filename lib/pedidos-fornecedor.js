...
function gerarPdfDevolucaoColetor(extras, usuario) {
  ...
  fs.mkdirSync(DIR_RECEB, { recursive: true });
  const destino = path.join(DIR_RECEB, `${extras.id || nota.nNota || 'nota'}-devolucao.pdf`);
  ...
  const cols = [['Origem', 70], ['Código', 90], ['Produto', W - 70 - 90 - 70 - 200], ['Qtd', 70], ['Motivo', 200]];
  ...
  for (const [k, r] of itens.entries()) {
    ...
    for (const [val, w] of [[DEV_ORIGEM[r.origem] || r.origem, 70], [r.cod, 90], [r.descricao, cols[2][1]], [n3(r.qtd) + ' un', 70], [motivo, 200]]) { doc.text(String(val), xx + 3, y, { width: w - 6, lineBreak: false }); xx += w; }
    y += 16;
  }
...
// Recebimento em vez de recusasLoja(p, ln) — o caminho antigo (sem extras) continua igual.
