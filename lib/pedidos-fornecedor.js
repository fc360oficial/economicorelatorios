...
async function anexarAvarias(p) {
  if (p.teste) { if (p.avarias?.itens) p.avarias = { ...p.avarias, ...resumoAvarias(p.avarias.itens), itens: p.avarias.itens }; incorporarAvarias(p); return salvar(p); }
...
function textoAvarias(p, ln) {
  ...
  const L = ['AVARIAS PENDENTES com o fornecedor (' + its.length + (its.length > 1 ? ' itens' : ' item') + ' · R$ ' + Number(its.reduce((s, a) => s + a.total, 0)).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) + '). As reposições já estão no pedido, marcadas "reposição de avaria": bonificar ou lançar preço zero.'];
...
function criarTestesAvaria(usuario) {
  ...
  const mk = (caso, titulo, avarias) => {
    ...
    incorporarAvarias(p); return salvar(p);
  };
...
function visaoVendedor(p) {
  return {
    ...
    itens: p.itens.map(i => ({ cod: i.cod, descricao: i.descricao, unid: i.unid, emb: i.emb, qtd: i.qtd, volumes: i.volumes, lojas_qtd: i.lojas_qtd, preco: i.preco, obs: i.obs, avaria: i.avaria ? Object.fromEntries(Object.entries(i.avaria).map(([l, a]) => [l, { qtd: a.qtd_pedido, status: a.status }])) : null, so_avaria: !!(i.avaria && Object.keys(i.lojas_qtd).every(l => i.avaria[l] && i.avaria[l].qtd_pedido >= (i.lojas_qtd[l] || 0))) }))
  };
}
