...
function gerarPdf(p) {
  ...
  try { doc.image(path.join(__dirname, '..', 'public', 'logo.png'), 28 + W - 52, 33, { height: 44 }); } catch (e) {}
  doc.fillColor('#FFC933').font('Helvetica-Bold').fontSize(9).text('PEDIDO DE COMPRA APROVADO · ECONOMICO SUPERMERCADO - REDE CAHU', 40, 36);
  doc.fillColor('#FFFFFF').fontSize(15).text(`Pedido #${p.id} · ${p.lista_nome}`, 40, 48);
