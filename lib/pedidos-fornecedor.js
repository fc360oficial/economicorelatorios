...
//   5. no pedido finalizado a comprador(a) vê último custo × preço digitado com
...
// "Gerar com Custo" (Dlinks: MultLoja=1): a comprador(a) fecha o pedido na hora com o custo como preço, sem link do vendedor.
...
function observacoesPedido(p) {
  ...
  if (p.obs_fiscal) out.push({ chave: 'compradora', rotulo: 'Observação da comprador(a) (Fiscal e fornecedor)', texto: p.obs_fiscal, por: p.obs_fiscal_por || null });
...
// status "sugestao" até a comprador(a) mandar pro vendedor.
// ─────────────────────────────────────────────────────────────
...
// Decisão da comprador(a) ANTES do caminhão chegar (o XML entra no ERP antes da entrega):
...
function recusasLoja(p, ln) {
  ...
  for (const i of x.itens || []) {
    ...
    else out.push({ cod: i.cod, descricao: i.descricao, qtd: i.recebida, motivo: 'recusado pela comprador(a)', preco: i.preco_xml });
  }
