...
function gerarPdf(p, soLoja) {
  ...
  if (p.obs_fiscal) {
    ...
    const alt = Math.max(22, doc.heightOfString('OBSERVAÇÃO DA COMPRADORA (fornecedor e recebimento): ' + p.obs_fiscal, { width: W - 16 }) + 12);
    doc.roundedRect(28, y, W, alt, 4).lineWidth(0.6).fillAndStroke('#FFF6D9', '#EFD98F');
    doc.fillColor('#6B4E00').font('Helvetica-Bold').fontSize(8.5).text('OBSERVAÇÃO DA COMPRADORA (fornecedor e recebimento): ', 36, y + 6, { continued: true, width: W - 16 }).font('Helvetica').fillColor('#0E1626').text(p.obs_fiscal, { width: W - 16 });
    y += alt + 8;
  }
  // resumo por loja
  ...
  if (p.obs_vendedor) { doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#0E1626').text('Observação do vendedor: ', 36, y, { continued: true }).font('Helvetica').text(p.obs_vendedor, { width: W }); y += 18; }
