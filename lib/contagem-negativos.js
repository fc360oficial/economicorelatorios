...
function visaoCentral(data) {
  ...
  const lojas = Object.keys(LOJAS_NOMES).map(ln => {
    ...
    return { loja: +ln, nome: l.nome, itens: l.itens.length, contados, zerados: linhas.filter(x => x.zero).length, status, resp: l.resp, iniciadoEm: l.iniciadoEm, concluidoEm: l.concluidoEm, ajustadoEm: l.ajustadoEm || null, ajustadoPor: l.ajustadoPor || null,
...
module.exports = { init, hojeStr, listarDias, obter, abrirDia, visaoLoja, diaAtualDaLoja, lancarItem, concluir, reabrir, marcarAjustado, visaoCentral, csv, config, lojaPorPin, lojaPorToken, LOJAS_NOMES };
