...
function reabrir(data, ln) {
  ...
  d.lojas[ln].ajustadoEm = null; d.lojas[ln].ajustadoPor = null; // contagem vai mudar, ajuste no ERP precisa ser refeito
  d.lojas[ln].aprovadoEm = null; d.lojas[ln].aprovadoPor = null; d.lojas[ln].pendentes = []; // idem aprovação no teste (o histórico fica no Log)
  salvar(d);
  return visaoLoja(data, ln);
}

// Central marca que já pegou a contagem e ajustou o estoque no ERP (feito por quem, quando)
function marcarAjustado(data, ln, nome, desfazer = false) {
  const d = obter(data);
  if (!d || !d.lojas[ln]) throw new Error('Contagem não encontrada.');
  const l = d.lojas[ln];
  if (desfazer) { l.ajustadoEm = null; l.ajustadoPor = null; }
  else {
    if (l.status !== 'concluida') throw new Error('A loja ainda não concluiu a contagem.');
    l.ajustadoEm = agora(); l.ajustadoPor = nome || null;
  }
  salvar(d);
  return { ok: true, ajustadoEm: l.ajustadoEm, ajustadoPor: l.ajustadoPor };
}

// Aprovação do ajuste no MySQL de teste (.254): guarda quem/quando, ids do Log e os códigos que falharam.
function marcarAprovado(data, ln, { nome, ids = [], pendentes = [] }) {
  const d = obter(data);
  if (!d || !d.lojas[ln]) throw new Error('Contagem não encontrada.');
  const l = d.lojas[ln];
  if (l.status !== 'concluida') throw new Error('A loja ainda não concluiu a contagem.');
  l.aprovadoEm = agora(); l.aprovadoPor = nome || null;
  l.aprovadoIds = [...(l.aprovadoIds || []), ...ids];
  l.pendentes = pendentes;
  salvar(d);
  return { ok: true, aprovadoEm: l.aprovadoEm, aprovadoPor: l.aprovadoPor, pendentes: l.pendentes };
}
...
function visaoCentral(data) {
  ...
  const lojas = Object.keys(LOJAS_NOMES).map(ln => {
    const l = d.lojas[ln];
    if (!l) return { loja: +ln, nome: LOJAS_NOMES[ln], semNegativos: true, itens: 0, contados: 0, zerados: 0, status: 'sem_negativos', resp: null, iniciadoEm: null, concluidoEm: null, ajustadoEm: null, ajustadoPor: null, linhas: [] };
    ...
    return { loja: +ln, nome: l.nome, itens: l.itens.length, contados, zerados: linhas.filter(x => x.zero).length, status, resp: l.resp, iniciadoEm: l.iniciadoEm, concluidoEm: l.concluidoEm, ajustadoEm: l.ajustadoEm || null, ajustadoPor: l.ajustadoPor || null, aprovadoEm: l.aprovadoEm || null, aprovadoPor: l.aprovadoPor || null, pendentes: l.pendentes || [],
...
function csv(data) {
  ...
  const out = [['Data', 'Loja', 'Nome Loja', 'Codigo', 'Descricao', 'Grupo', 'SubGrupo', 'Sistema', 'Deposito', 'Loja (area de venda)', 'Contado', 'Diferenca', 'Nao achei', 'Contado por', 'Contado em', 'Ajustado por', 'Ajustado em'].join(';')];
  for (const l of v.lojas) for (const x of l.linhas)
    out.push([v.data, l.loja, l.nome, x.cod, x.desc, x.grupo, x.sub, x.sys, x.dep ?? '', x.loja ?? '', x.contado ?? '', x.diferenca ?? '', x.zero ? 'SIM' : '', x.por ?? '', x.em ? x.em.slice(0, 16).replace('T', ' ') : '', l.ajustadoPor ?? '', l.ajustadoEm ? l.ajustadoEm.slice(0, 16).replace('T', ' ') : ''].map(esc).join(';'));
  return '﻿' + out.join('\r\n');
}

module.exports = { init, hojeStr, listarDias, obter, abrirDia, visaoLoja, diaAtualDaLoja, lancarItem, concluir, reabrir, marcarAjustado, marcarAprovado, visaoCentral, csv, config, lojaPorPin, lojaPorToken, LOJAS_NOMES };
