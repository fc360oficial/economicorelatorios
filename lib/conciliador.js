...
// Devedor > 0 sozinho não distingue "ninguém mexeu nesse título" de "o
// pagamento já bateu com o banco no ERP (loja20045.contasapagarbaixaconta),
// só falta o clique de baixa definitiva no título" — descoberto direto no
// banco em 2026-09-03 comparando um título confirmado pelo Tiago no ERP
// (Histórico Documento mostrava "Valor Em Aberto: 0,00" com o pagamento
// batido, mas Devedor continuava cheio). BaixaLancada (subquery em
// server.js) marca esse caso intermediário.
function statusFinal(c) {
  if (Number(c.Devedor) === 0) return 'conciliado';
  return Number(c.BaixaLancada) > 0 ? 'baixa_pendente' : 'pago_sem_baixa';
}
...
function conciliar(saidas, candidatos) {
  ...
  return saidas.map(saida => {
    ...
    if (!dentroTolerancia.length) {
      const porEncargo = acharPorEncargoExato(saida, candidatos, usados);
      if (porEncargo) {
        usados.add(porEncargo.nReg);
        return { ...saida, status: statusFinal(porEncargo), match: { ...montarMatch(porEncargo, saida.data, saida.favorecido, saida.valor), casadoPorEncargo: true }, candidatos: [] };
      }
...
// Quando o valor do extrato não bate com o campo Valor do título, mas bate
// EXATO com Valor + acréscimo/multa/juros − desconto/devolução já lançados
// nesse mesmo título — não é uma divergência, é o título batendo certinho
// (achado 2026-09-08: o campo Valor às vezes não reflete os encargos
// lançados depois, mas eles já estão registrados no próprio ERP). Só assume
// automaticamente quando o nome bate razoavelmente e não há ambiguidade —
// senão cai em "divergência"/"revisar" normalmente, tratado fora desta função.
function acharPorEncargoExato(saida, candidatos, usados) {
  const EPSILON = 0.01;
  const pontuados = candidatos
    .filter(c => !usados.has(c.nReg))
    .filter(c => diffDias(saida.data, c.DataVencto) <= TOLERANCIA_DIAS)
    .map(c => {
      const efeito = (Number(c.Acrescimo) || 0) + (Number(c.Multa) || 0) + (Number(c.Juros) || 0) - (Number(c.Desconto) || 0) - (Number(c.Devolucao) || 0);
      return { c, efeito, esperado: Number(c.Valor) + efeito, sim: similaridadeNome(saida.favorecido, c.NomeCompleto || c.Nome) };
    })
    .filter(x => x.efeito !== 0 && Math.abs(saida.valor - x.esperado) <= EPSILON && x.sim > 0.5)
    .sort((a, b) => b.sim - a.sim);

  if (!pontuados.length) return null;
  if (pontuados.length > 1 && pontuados[0].sim - pontuados[1].sim < 0.2) return null;
  ...
  return itens.map(it => {
    ...
    return {
      ...
      match: {
        ...
        acrescimo: av.acrescimo || 0,
        multa: av.multa || 0,
        juros: av.juros || 0,
        desconto: av.desconto || 0,
        devolucao: av.devolucao || 0,
        valorBruto: av.valorBruto != null ? av.valorBruto : null,
