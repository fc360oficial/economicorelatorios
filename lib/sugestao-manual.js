...
async function preencherUltimaVenda(s) {
  ...
  await Promise.all(s.parametros.lojas.map(async ln => {
    const ult = {}, diasV = {};
    ...
    for (const m of [...new Set(mesesDoPeriodo(dIni, dFim))]) {
      try {
        const rows = await q(`SELECT Codigo, MAX(Data) ultima, COUNT(DISTINCT CASE WHEN Data <= ? THEN Data END) dias FROM \`ln${ln}${mesDB(m)}\`.zcupomitens WHERE Data BETWEEN ? AND ? AND IndCancel='N' AND Codigo IN (${ph}) GROUP BY Codigo`, [dFimPer, dIni, dFim, ...cods]);
        for (const r of rows) { const d = fmt(r.ultima); if (d) ult[String(r.Codigo)] = d; diasV[String(r.Codigo)] = (diasV[String(r.Codigo)] || 0) + (parseInt(r.dias) || 0); }   // meses em ordem: o último mês com venda prevalece
...
async function preencherVAC(s, dFim) {
  ...
  await Promise.all(s.parametros.lojas.map(async ln => {
    ...
    const cods = Object.keys(inicio); if (!cods.length) return;
    ...
    for (const m of [...new Set(mesesDoPeriodo(minIni, dFim))]) {   // cada banco mensal só uma vez (rotativo de 12 meses)
      try {
        const rows = await q(`SELECT Codigo, SUM(QtdNovo) q FROM \`ln${ln}${mesDB(m)}\`.zcupomitens WHERE Data BETWEEN ? AND ? AND IndCancel='N' AND Codigo IN (${ph}) AND Data >= ${caso} GROUP BY Codigo`, [minIni, dFim, ...cods, ...casoParams]);
        for (const r of rows) vac[String(r.Codigo)] = (vac[String(r.Codigo)] || 0) + num(r.q);
      } catch (e) { console.error('[SUGESTAO-MANUAL] VAC mes ' + m + ' loja ' + ln + ':', e.message); }
    }
