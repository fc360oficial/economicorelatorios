...
// Cor "Avarias" do Dlinks: o produto teve lançamento em central.avariaconsumo (DataLan) em alguma loja da sugestão
// dentro do período de vendas. Marca it.avaria = { lojas:[..], qtd, total } e l.avaria = { qtd, total } por loja.
async function preencherAvarias(s) {
  ...
  const rows = await q(`SELECT nLoja, CodigoBarras, SUM(Qtd) qtd, SUM(Total) total, COUNT(*) n FROM central.avariaconsumo
    ...
    GROUP BY nLoja, CodigoBarras`, [...lojas, dIni, dFim, ...cods]);
  const m = {};
...
module.exports = { DIR, _setDir, init, proximoId, salvar, obter, listar, aplicarPatch, calcularLoja, repartirPorLoja, initERP, montarDeLinhas, montarDoERP, montarItens, mapaABC, abcDoM, preencherAvarias, mesesDoPeriodo, calcularNova, recalcular, num };
