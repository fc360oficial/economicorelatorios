...
module.exports = function montarRotasRecebimento(app, deps) {
  ...
  app.get('/api/recebimento/:id/devolucao/pdf', async (req, res) => {
    ...
    let f;
    try {
      f = pedidosFornec.gerarPdfDevolucao(null, c.loja, req.session?.user?.nome || null,
        { id: c.id, itens, nota: { nNota: c.nNota, chave: c.chave, fornecedor: c.fornecedor }, loja: c.loja, motorista: true });
    } catch (e) { return res.status(500).json({ error: e.message }); }
    await new Promise(r => setTimeout(r, 250));   // pdfkit escreve o arquivo em stream (mesmo padrão das rotas de pedido)
    res.sendFile(f, { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `inline; filename="devolucao-${c.id}.pdf"` } });
  });
