...
function devolucoes(c) { const x = xmlDe(c); const out = [];
  for (const xi of x.itens || []) if (xi.decisao && xi.decisao.acao === 'recusar') out.push({ cod: xi.cod, descricao: xi.descricao, qtd: xi.un, origem: 'compras', motivo: 'recusado pela comprador(a) na conferência XML' });
