...
const PADRAO = { validade_pct_min: 100, recontagens_min: 1, modo_cega: 'total', janela_dias_axml: 7, fiscal_ativo: false }; // fiscal_ativo: liga a integração no Fiscal (desligada até os testes do Tiago)
...
const ontemStr = () => diaStr(new Date(agora().getTime() - 864e5));
// id de conferencia = AAAA-MM-DD-<loja>-<hash curto da chave da NF-e>. A validação existe porque o id
// vira nome de arquivo (lerDia usa id.slice(0,10)): sem ela um id com ".." sairia de DIR.
const RE_ID = /^\d{4}-\d{2}-\d{2}-[1-6]-[\w.-]{1,60}$/;
const RE_DIA = /^\d{4}-\d{2}-\d{2}$/;
function erro400(msg) { const e = new Error(msg); e.status = 400; return e; }
function validarId(id) { if (!RE_ID.test(String(id || ''))) throw erro400('id de conferência inválido'); return String(id); }
function validarDia(data) { if (!RE_DIA.test(String(data || ''))) throw erro400('data inválida'); return String(data); }
...
function obter(id) { validarId(id); return lerDia(id.slice(0, 10)).confs[id] || null; }
function salvar(c) { validarId(c.id); const d = lerDia(c.id.slice(0, 10)); c.atualizadoEm = agora().toISOString(); d.confs[c.id] = c; gravarDia(d); return c; }
function listarDia(data = hojeStr()) { return Object.values(lerDia(data).confs); }
// A conferência é arquivada por dia, mas uma nota aberta às 23h50 continua sendo bipada depois da
// meia-noite: procurar em ontem+hoje evita abrir uma 2a conferência (e um 2o INSERT no ERP).
function acharPorChave(loja, chave) {
  if (!chave) return null;
  for (const dia of [hojeStr(), ontemStr()]) {
    const c = listarDia(dia).find(x => x.chave === chave && +x.loja === +loja);
    if (c) return c;
  }
  return null;
}
function abrirNota({ loja, nome, chave, nNota, fornecedor, codFornec, xmlItens }) {
  const ex = acharPorChave(loja, chave);
  if (ex) { if (!(ex.xml_itens || []).length && (xmlItens || []).length) { ex.xml_itens = xmlItens; salvar(ex); } return ex; }
  // hash da chave (e não o no da nota): dois fornecedores podem emitir a mesma NF no mesmo dia.
  const id = `${hojeStr()}-${loja}-${crypto.createHash('sha1').update(String(chave)).digest('hex').slice(0, 10)}`;
  return salvar({ id, loja: +loja, nome: String(nome || '').toUpperCase().slice(0, 20), chave, nNota: String(nNota), fornecedor, codFornec: +codFornec || 0, status: 'bipando', abertoEm: agora().toISOString(), itens: {}, recontagens: 0, mensagens: [], xml_itens: xmlItens || [], erp: { nReg: null, erros: [] } });
}
...
// Itens da nota: preferir o que foi lido do XML do ERP na abertura (c.xml_itens, existe até pra nota
// sem pedido do app). O pedido do app entra so como OVERLAY da decisão do(a) comprador(a) (recusar).
// c.xml_itens NUNCA sai em visaoLoja nem em GET /api/recebimento — a conferência continua cega.
function xmlDe(c) {
  const ped = deps.xmlLoja ? deps.xmlLoja(c.chave) : null;
  const nota = c.xml_itens || [];
  if (!nota.length) return ped || { itens: [], naoPedidos: [], status: 'sem_pedido' };
  const dec = new Map(((ped && ped.itens) || []).concat((ped && ped.naoPedidos) || []).map(i => [i.cod, i.decisao || null]));
  return {
    pedidoId: ped ? ped.pedidoId : null,
    status: ped ? ped.status : 'sem_pedido',
    naoPedidos: (ped && ped.naoPedidos) || [],
    itens: nota.map(i => ({ cod: i.cod, descricao: i.descricao, un: +i.un || 0, decisao: i.decisao || dec.get(i.cod) || null })),
  };
}
// O coletor manda um bipeId por bipe/correção. Se a rede cai depois do servidor aplicar mas antes
// da resposta chegar, a fila offline reenvia o mesmo bipeId — aqui a gente devolve o resultado
// guardado em vez de somar a quantidade de novo.
const LIMITE_BIPES_VISTOS = 500;
function jaAplicado(c, bipeId) { return bipeId && c.bipes_vistos && c.bipes_vistos[bipeId] ? c.bipes_vistos[bipeId] : null; }
function respostaRepetida(c, v) { return { resultado: v.resultado, item: c.itens[v.cod] || null, repetido: true }; }
function marcarAplicado(c, bipeId, resultado, cod) {
  if (!bipeId) return;
  c.bipes_vistos = c.bipes_vistos || {};
  c.bipes_vistos[bipeId] = { resultado, cod };
  const ks = Object.keys(c.bipes_vistos);
  if (ks.length > LIMITE_BIPES_VISTOS) for (const k of ks.slice(0, ks.length - LIMITE_BIPES_VISTOS)) delete c.bipes_vistos[k];
}
async function bipar(id, { cod, quant, emb, validade, nome, bipeId }) {
  ...
  const visto = jaAplicado(c, bipeId); if (visto) return respostaRepetida(c, visto);
  ...
  // dois critérios: a régua relativa do coletor (Validar x pct) e o PISO absoluto do Fiscal
  // (validade_min_dias) — qualquer item que vença antes do piso é bloqueado, tenha cadastro ou não.
  const piso = deps.validadeMinDias ? (+deps.validadeMinDias() || 0) : 0;
  if (resultado === 'ok' && d != null && piso > 0 && d < piso) { it.estado = 'bloqueado_validade'; it.origem_devolucao = 'coletor'; resultado = 'bloqueado_validade'; }
  else if (resultado === 'ok' && it.validar > 0 && d != null && d < it.validar * (cfg.validade_pct_min / 100)) { it.estado = 'bloqueado_validade'; it.origem_devolucao = 'coletor'; resultado = 'bloqueado_validade'; }
  ...
  it.por = nome || c.nome; it.em = agora().toISOString(); marcarAplicado(c, bipeId, resultado, cod); salvar(c); return { resultado, item: it };
}
async function corrigir(id, { cod, quant, emb, validade, bipeId }) { const c = obter(id); const it = c && c.itens[cod]; if (!it) throw new Error('Item não bipado');
  const visto = jaAplicado(c, bipeId); if (visto) return respostaRepetida(c, visto);
  it.quant = 0; it.un = 0; it.validade = null; it.bipagens = 0; delete it.origem_devolucao; delete it.aviso; salvar(c); return bipar(id, { cod, quant, emb, validade, bipeId, nome: c.nome }); }
...
function enviarAssimMesmo(id) { const c = obter(id); if (!c) throw new Error('Conferência não encontrada'); if (c.status === 'liberada') throw new Error('Nota já liberada'); if (c.recontagens < config().recontagens_min) throw new Error('Reconte antes de enviar'); c.status = 'terminada'; return salvar(c); }
function devolucoes(c) { const x = xmlDe(c); const out = [];
  ...
  return out; }
const LIMITE_MSGS_VISTOS = 200;
function mensagem(id, m) { const c = obter(id); if (!c) throw new Error('Conferência não encontrada');
  // mesma ideia do bipeId: a fila offline do coletor reenvia a mensagem quando a rede volta.
  if (m.msgId) { c.msgs_vistos = c.msgs_vistos || {}; const j = c.msgs_vistos[m.msgId]; if (j) return { ...j, repetido: true }; }
  const msg = { em: agora().toISOString(), de: m.de, nome: m.nome, motivo: m.motivo || null, texto: m.texto || null, cod: m.cod || null, acao: m.acao || null };
  ...
  if (m.msgId) { c.msgs_vistos[m.msgId] = msg; const ks = Object.keys(c.msgs_vistos); if (ks.length > LIMITE_MSGS_VISTOS) for (const k of ks.slice(0, ks.length - LIMITE_MSGS_VISTOS)) delete c.msgs_vistos[k]; }
  salvar(c); return msg; }
...
module.exports = { init, config, setConfig, lojaPorPin, lojaPorToken, hojeStr, ontemStr, validarId, validarDia, acharPorChave, obter, salvar, listarDia, abrirNota, bipar, corrigir, visaoLoja, diasAte, xmlDe, LOJAS, terminei, enviarAssimMesmo, devolucoes, mensagem, liberar, reconferir };
