...
/** Separa os itens de uma loja (saída de visaoCentral) em "gravar" (com contagem) e "semContagem". */
function itensParaGravar(linhas, somenteCods = null) {
  const so = somenteCods ? new Set(somenteCods.map(String)) : null;
  const gravar = [], semContagem = [];
