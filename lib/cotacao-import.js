...
function casar(linhas, fornecedores) {
  ...
  for (const f of fornecedores || []) {
    ...
    const item = { codFornec: cod, nome, curto, n1: norm(nome), n2: norm(curto), t: new Set(tokens(nome).concat(tokens(curto))) };
    lista.push(item);
  ...
  return linhas.map(l => {
    ...
    if (!m) {
      ...
      for (const f of lista) {
        ...
        else if (tk.length) { const comum = tk.filter(t => f.t.has(t)).length; const j = comum / Math.max(tk.length, 1); if (comum >= 1 && j >= 0.6) s = 0.5 + j * 0.3; else if (comum >= 2) s = 0.55; }
        if (s > bs) { bs = s; best = f; }
      }
      if (best && bs >= 0.5) { m = best; como = bs >= 0.9 ? 'nome' : 'parcial'; score = bs; }
    }
    return { ...l, codFornec: m ? m.codFornec : 0, nome_erp: m ? m.nome : null, como, score: +score.toFixed(2) };
  });
}
