...
function gerarModeloPdf(ponta, res) {
  ...
  doc.moveDown(1.6);
  doc.fontSize(9.5).text(`${cidade}, ${dataHojeExtenso()}`, { align: 'center' });

  doc.moveDown(4);
  const y1 = doc.y;
  doc.moveTo(176, y1).lineTo(420, y1).stroke();
  doc.fontSize(9.5).text(razaoSocial, 176, y1 + 4, { width: 244, align: 'center' });
  doc.text(cnpjLoja, 176, doc.y, { width: 244, align: 'center' });

  doc.moveDown(2);
  const y2 = doc.y;
  doc.moveTo(176, y2).lineTo(420, y2).stroke();
  doc.text(fornecedor, 176, y2 + 4, { width: 244, align: 'center' });
