...
// DESLIGADO em 14/09/2026 após teste real: mês-a-mês do ano passado é ruído (escova dental ×3, papel ×2,9), HOJE foi de 39 pra 100 listas
// e o recálculo de 100 s pra 496 s. Volta quando existir a versão com 2 anos, sazonalidade consistente nos dois anos e cache em disco.
const HIST12_ATIVO = false;
const SAZONAL_MIN_UN = 30;   // venda mínima no mês-base (ano passado) pra confiar no fator
const SAZONAL_LIMIAR = 0.25; // só aplica se o fator sair de 0,75–1,25
...
async function coletarBase() {
  ...
  if (HIST12_ATIVO && (!hist12Cache || dd(hoje, hist12Cache.calculadoEm) >= 1)) { console.log('[RADAR] histórico 12 meses por loja…'); hist12Cache = await calcularHist12(codes, hoje); }
  const H = HIST12_ATIVO && hist12Cache ? hist12Cache.mensal : {};
  // meses de referência da sazonalidade: mês atual e mês que vem, no ANO PASSADO
  const dAtualLY = new Date(hoje + 'T00:00:00Z'); dAtualLY.setUTCFullYear(dAtualLY.getUTCFullYear() - 1);
  const dProxLY = new Date(dAtualLY); dProxLY.setUTCMonth(dProxLY.getUTCMonth() + 1);
  const mesAtualLY = ym(dAtualLY), mesProxLY = ym(dProxLY);
  ...
  for (const it of itens) {
    ...
    for (const ln of LOJAS) {
      ...
      porLoja[ln] = { vq: (v ? v.qtd : 0) / dias, vR: (v ? v.valor : 0) / dias, est: Math.max(0, eL), estBruto: eL, transito: tL, transitoDet: transitoDet[`${it.cod}|${ln}`] || [], fonte: '40d' };
      // sem venda nos 40 dias → média dos 12 meses DESSA loja (se houver)
      if (!(v && v.qtd > 0)) { const h = H[`${it.cod}|${ln}`]; if (h) { let q12 = 0, v12 = 0; for (const x of Object.values(h)) { q12 += x.qtd; v12 += x.valor; } if (q12 > 0) { porLoja[ln].vq = q12 / 365; porLoja[ln].vR = v12 / 365; porLoja[ln].fonte = '12m'; qtd += q12 / 365 * dias; val += v12 / 365 * dias; nFallback12++; } } }
    }
    // sazonalidade (todas as lojas): mês que vem ÷ mês atual, ambos no ano passado
    let sazonal = null;
    { let qAt = 0, qPx = 0; for (const ln of lojas) { const h = H[`${it.cod}|${ln}`]; if (!h) continue; qAt += h[mesAtualLY]?.qtd || 0; qPx += h[mesProxLY]?.qtd || 0; }
      if (qAt >= SAZONAL_MIN_UN && qPx > 0) { const f = Math.max(0.5, Math.min(3, qPx / qAt)); if (Math.abs(f - 1) >= SAZONAL_LIMIAR) { sazonal = { fator: +f.toFixed(2), base: mesAtualLY, alvo: mesProxLY, qtd_base: qAt, qtd_alvo: qPx }; for (const ln of lojas) { porLoja[ln].vq *= f; porLoja[ln].vR *= f; } qtd *= f; val *= f; nSazonal++; } } }
    const pm = qtd > 0 ? val / qtd : 0;
    prods.push({
      lista: it.nLista, cod: it.cod, descricao: it.descricao, unid: it.unid || 'UN', emb: num(it.emb) > 0 ? num(it.emb) : 1,
      validade: num(it.validade), lojas, porLoja, sazonal, fonte12: lojas.filter(l => porLoja[l].fonte === '12m'),
  ...
  console.log(`[RADAR] venda: ${nFallback12} loja×produto com média de 12 meses, ${nSazonal} produtos com ajuste sazonal (${mesAtualLY}→${mesProxLY})`);
  return { hoje, dIni, dFim, dias, listas, prods, embHist, hist12: { fallback12: nFallback12, sazonal: nSazonal, base: mesAtualLY, alvo: mesProxLY }, curvaA: { n: Object.keys(curvaA).length, fracao: CURVA_A_FRACAO, produtos: ordem.length, vendaDia: totR } };
}
...
async function calcularHist12(codes, hoje) {
  const { q, mesDB } = deps;
  const dIni = addDias(hoje, -365), dFim = addDias(hoje, -1);
  const mensal = {};
  for (const ln of LOJAS) for (let m = 1; m <= 12; m++) for (const c of chunk(codes, 4000)) {
    ...
    try {
      ...
      for (const x of r) { const k = `${x.cod}|${ln}`; (mensal[k] = mensal[k] || {})[x.mes] = { qtd: num(x.qtd), valor: num(x.valor) }; }
    } catch (e) {}
  }
...
function qtdPedido(p, P, fazerEm, embMeses = EMB_HIST_MESES_PADRAO) {
  ...
  for (const ln of p.lojas) {
    ...
    // Loja ZERADA sem venda: não vende porque não tem. Se o produto vende nas outras lojas e não há nada
    // em trânsito, sugere 1 caixa pra recolocar (regra do Tiago, 14/09/2026). Sem isso a ruptura se esconde.
    if (L.vq <= 0) { if (L.est + L.transito <= 0 && p.vq > 0) { porLoja[ln] = emb; total += emb; flag = flag || 'zerado'; } else porLoja[ln] = 0; continue; }
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true) {
  ...
  const itens = base.prods.filter(p => p.lista === +listaId).map(p => {
    ...
    return {
      ...
      lojas_qtd: r.porLoja || {},
      lojas_det: Object.fromEntries(p.lojas.map(ln => { const L = p.porLoja[ln]; return [ln, { estoque: +L.est.toFixed(1), transito: +L.transito.toFixed(1), transito_det: L.transitoDet || [], venda_dia: +L.vq.toFixed(3), fonte: L.fonte || '40d', cobertura_dias: L.vq > 0 ? +((L.est + L.transito) / L.vq).toFixed(1) : null }]; })),
...
module.exports = { init, agendar, recalcular, politica, itensLista, curvaARisco, sombra, sombraDetalhe, nfe, getEstado, curvaASet, TETO_PADRAO,
