...
async function coletarBase() {
  ...
  for (const it of itens) {
    ...
    { const soma = m => { let t = 0; for (const ln of lojas) t += H[`${it.cod}|${ln}`]?.[m]?.[0] || 0; return t; };
      ...
      const mediaMensal = ate => { const [y, m] = ate.split('-').map(Number); let t = 0; for (let k = 0; k < 12; k++) { const d = new Date(Date.UTC(y, m - 1 - k, 1)); t += soma(ym(d)); } return t / 12; };
      const a1 = soma(mesAtualLY), p1 = soma(mesProxLY), a2 = soma(mesAtualLY2), p2 = soma(mesProxLY2);
      const m1 = mediaMensal(mesProxLY), m2 = mediaMensal(mesProxLY2);
...
async function calcularHist12(codes, hoje) {
  const { q, mesDB } = deps;
  const dIni = addDias(hoje, -HIST_MESES * 30.5 | 0), dFim = addDias(hoje, -1);
  const mensal = {};
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true) {
  ...
  const itens = base.prods.filter(p => p.lista === +listaId).map(p => {
    ...
    return {
      ...
      lojas_qtd: r.porLoja || {},
      lojas_det: Object.fromEntries(p.lojas.map(ln => { const L = p.porLoja[ln]; return [ln, { estoque: +L.est.toFixed(1), estoque_bruto: +L.estBruto.toFixed(1), transito: +L.transito.toFixed(1), transito_det: L.transitoDet || [], venda_dia: +L.vq.toFixed(3), fonte: L.fonte || '40d', cobertura_dias: L.vq > 0 ? +((L.est + L.transito) / L.vq).toFixed(1) : null }]; })),
...
module.exports = { init, agendar, recalcular, politica, itensLista, curvaARisco, sombra, sombraDetalhe, nfe, atualizarHistEmSegundoPlano, getEstado, curvaASet, TETO_PADRAO,
