...
const VENDA_MIN_LOJA_ZERADA = 1; // un/dia: loja zerada só conta como risco se o produto vende pelo menos isso lá
...
function politica(teto = TETO_PADRAO, filtroComprador = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true) {
  ...
  for (const L of Object.values(base.listas)) {
    ...
    for (const p of comVenda) {
      ...
      if (zeraAntes || cob <= P.ponto) curvaARisco.push({ cod: p.cod, descricao: p.descricao, rank: p.rankA, cobertura_dias: +cob.toFixed(1), venda_dia_valor: +p.vR.toFixed(2), zera_antes: zeraAntes, fazer_em: Math.max(0, Math.round(cob - P.ponto)) });
    }
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true) {
  ...
  const itens = base.prods.filter(p => p.lista === +listaId).map(p => {
    ...
    const riscoA = !!(p.curvaA && P && cob != null && (cob < fazerEmLista || cob <= P.ponto));
    return {
      ...
      curva_a: !!p.curvaA, rank_a: p.rankA || null, risco_a: riscoA,
  ...
  }).sort((a, b) => b.risco_a - a.risco_a || (b.qtd > 0) - (a.qtd > 0) || (a.cobertura_dias ?? 1e9) - (b.cobertura_dias ?? 1e9));
...
function curvaARisco(teto = TETO_PADRAO, filtroComprador = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true) {
  ...
  for (const p of base.prods) {
    ...
    const cob = (p.est + p.transito) / p.vq;
    const zeraAntes = cob < r.fazer_em_lista, noPonto = cob <= r.ponto;
