...
async function coletarBase() {
  ...
  try {
    ...
    try {
      ...
      const pr = await q(`SELECT x.Chave chave, p.CodigoBarras cb, p.ocEanTrib ean FROM central.axml x JOIN central.axmlprodutos p ON p.nNota=x.nNota AND p.CNPJemit=x.CNPJemit WHERE x.nMod='55' AND x.Importado=0 AND x.Data>=?`, [addDias(hoje, -30)]);
...
function projetarLoja(prods, loja) {
  const ln = parseInt(loja); if (!(ln > 0)) return prods;
  const out = [];
  for (const p of prods) {
    const L = p.porLoja && p.porLoja[ln]; if (!L || !p.lojas.includes(ln)) continue;
    out.push({ ...p, lojas: [ln], porLoja: { [ln]: L }, vq: L.vq, vR: L.vR, est: L.est, estBruto: L.estBruto, transito: L.transito, fonte12: (p.fonte12 || []).filter(l => l === ln) });
  }
  return out;
}
function politica(teto = TETO_PADRAO, filtroComprador = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true, loja = null) {
  if (!base) return [];
  const byL = {};
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true, paramsPadrao = null, opts = null) {
  ...
  const loja = opts && opts.loja ? parseInt(opts.loja) : null;
  const row = politica(teto, null, embMeses, usarCurvaA, loja).find(r => r.lista === +listaId);
...
function curvaARisco(teto = TETO_PADRAO, filtroComprador = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true, loja = null) {
  if (!base) return { resumo: null, itens: [] };
  const rows = politica(teto, filtroComprador, embMeses, usarCurvaA, loja);
...
module.exports = { init, agendar, recalcular, recarregarEmbPadrao, calcularHist12, politica, itensLista, curvaARisco, projetarLoja, sombra, sombraDetalhe, nfe, atualizarHistEmSegundoPlano, getEstado, curvaASet, TETO_PADRAO,
  // funções puras reaproveitadas por lib/pedidos-cd.js
  paramsLista, alvoProduto, qtdPedido, num, chunk,
  _setBaseParaTeste: b => { base = b; } };
