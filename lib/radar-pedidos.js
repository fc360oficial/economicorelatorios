...
function itensLista(listaId, teto = TETO_PADRAO) {
  if (!base) return null;
  const L = base.listas[listaId]; if (!L) return null;
  const lead = leadCache?.porLista?.[listaId];
  const P = paramsLista(L, lead, teto);
  const row = politica(teto).find(r => r.lista === +listaId);
  const fazerEm = row?.fazer_em ?? 0;
...
module.exports = { init, agendar, recalcular, politica, itensLista, sombra, getEstado, TETO_PADRAO };
