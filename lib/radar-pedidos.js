...
function politica(teto = TETO_PADRAO, filtroComprador = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true, loja = null, gatilho = null, _semLojas = false) {
  ...
  const porLojaDia = {};
  if (!loja && !_semLojas) {
    for (const ln of LOJAS_RADAR) for (const r of politica(teto, filtroComprador, embMeses, usarCurvaA, ln, FRACAO, true)) { if (!r.ok) continue; (porLojaDia[r.lista] = porLojaDia[r.lista] || {})[ln] = r.fazer_em; }
  }
  const byL = {};
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO, usarCurvaA = true, paramsPadrao = null, opts = null) {
  ...
  const loja = opts && opts.loja ? parseInt(opts.loja) : null, gatilho = opts && opts.gatilho > 0 ? +opts.gatilho : null;
