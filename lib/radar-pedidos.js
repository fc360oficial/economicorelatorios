...
async function fingerprintListas() {
  try { const [r] = await deps.q(`SELECT COUNT(*) n, COALESCE(SUM(nCotacao),0) s, COALESCE(SUM(CAST(Codigobarra AS UNSIGNED) % 9973),0) h FROM central.c_cotacao_lista_itens`); return [r.n, r.s, r.h].join('|'); }
  catch (e) { return null; }
}
function agendar() {
