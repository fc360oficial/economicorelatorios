...
function qtdPedido(p, P, fazerEm, embMeses = EMB_HIST_MESES_PADRAO) {
  ...
  for (const ln of p.lojas) {
    ...
    // Loja ZERADA sem venda: se a loja está MARCADA NA LISTA pra esse produto e está com estoque zero e nada em
    // trânsito, sugere 1 caixa (regra do Tiago, 14/09/2026: "dentro da lista da loja, não das outras lojas").
    if (L.vq <= 0) { if (L.est + L.transito <= 0) { porLoja[ln] = emb; total += emb; flag = flag || 'zerado'; } else porLoja[ln] = 0; continue; }
