...
const EMB_HIST_MESES_PADRAO = 12; // janela default usada pra decidir a embalagem real
...
function embCompra(cod, meses) {
  if (!meses || !base?.embHist?.[cod]) return null;
  const cont = {};
  for (const [e, idade, n] of base.embHist[cod]) if (idade < meses) cont[e] = (cont[e] || 0) + n;
...
async function coletarBase() {
  ...
  try {
    ...
    for (const c of chunk(codes, 4000)) {
      const r = await q(`SELECT CodigoBarra cod, QtdEmb emb, TIMESTAMPDIFF(MONTH, DataEntrada, CURDATE()) idade, COUNT(*) n
                         FROM central.compraprodutos
                         WHERE DataEntrada >= ? AND Movimentacao='COMPRA' AND QtdEmb > 0 AND CodigoBarra IN (${c.map(() => '?').join(',')})
                         GROUP BY CodigoBarra, QtdEmb, idade`, [dEmb, ...c]);
      for (const x of r) { const e = num(x.emb); if (e >= 1) (embHist[x.cod] = embHist[x.cod] || []).push([e, +x.idade || 0, +x.n]); }
    }
...
function qtdPedido(p, P, fazerEm, embMeses = EMB_HIST_MESES_PADRAO) {
  ...
  if (p.vq <= 0) return { qtd: 0, flag: null, emb };
  ...
  if (falta <= 0) return { qtd: 0, flag: null, alvo, emb };
  let flag = null;
  ...
  const qtd = Math.ceil(falta / emb) * emb;
  return { qtd, flag, alvo, emb };
}
...
function itensLista(listaId, teto = TETO_PADRAO, fazerEmOverride = null, embMeses = EMB_HIST_MESES_PADRAO) {
  ...
  const itens = base.prods.filter(p => p.lista === +listaId).map(p => {
    const r = P ? qtdPedido(p, P, fazerEm, embMeses) : { qtd: 0, flag: null, emb: embEfetiva(p, embMeses) };
    const cob = p.vq > 0 ? (p.est + p.transito) / p.vq : null;
    const embH = embCompra(p.cod, embMeses);
    return {
