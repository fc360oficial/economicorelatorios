...
function calcularItem(e, params) {
  ...
  else if (pa < ci - 1e-9) { out.preco_sugerido = arred(ci, term); out.piso = true; }   // piso absoluto vale também em mantem/sem_mudanca
  ...
  if (e.margem_atacado > 0) {
    ...
    let sug, pisoAt;
    if (status === 'sobe' || status === 'desce' || paAt == null) { sug = at.preco_sugerido; pisoAt = at.piso; }
    else if (paAt < ci - 1e-9) { sug = arred(ci, term); pisoAt = true; }                // idem no atacado
    else { sug = paAt; pisoAt = false; }
    out.atacado = { preco_calc: at.preco_calc, preco_sugerido: sug, preco_final: sug, piso: pisoAt };
  }
  return out;
}
