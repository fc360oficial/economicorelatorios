...
// Ajuste de centavos (Tiago, 24/09): dentro da terminação, alguns finais viram outros — "89>99,09>19" = preço que
// cair em ,89 vira ,99 e ,09 vira ,19. É um padrão que ele vai ampliando; texto "de>para" separado por vírgula,
// configurável no padrão e por pedido (flag na tela). Vazio = desligado.
const AJUSTE_CENTAVOS_PADRAO = '89>99,09>19';
function parseAjuste(txt) { const m = {}; String(txt || '').split(',').forEach(p => { const x = p.trim().match(/^(\d{1,2})\s*>\s*(\d{1,2})$/); if (x) m[+x[1]] = +x[2]; }); return m; }
function aplicarAjuste(v, m) { if (!m || !Object.keys(m).length || !(v > 0)) return v; const c = cents(v), cc = c % 100; if (m[cc] == null) return v; return (c - cc + m[cc]) / 100; }
function validaAjuste(txt) { return txt === '' || /^\s*\d{1,2}\s*>\s*\d{1,2}\s*(,\s*\d{1,2}\s*>\s*\d{1,2}\s*)*$/.test(String(txt)); }

const r2 = v => Math.round(v * 100) / 100;
const r4 = v => Math.round(v * 10000) / 10000;

// Tiago (24/09): a "margem de cadastro" do ERP (itens_margens.MargemVarejo) é margem SOBRE A VENDA; o preço sai pelo
// MARKUP divisor: preço = custo ÷ (1 − margem). Conferido no ERP: com 40% os preços reais ficam em ~1,61× o custo
// (divisor 1,667), não 1,40×. Antes usávamos custo × (1 + margem), que dava preço menor que o ERP pratica.
function precoPor(custo, margem, term) {
  const m = Math.min(Number(margem) || 0, 90) / 100;   // trava em 90% pra não explodir o preço com cadastro errado
  const calc = r2(custo / (1 - m));
...
//   3) custo novo MAIS DE 10% ABAIXO do atual (queda ≥ 10,01%) → preço 10% abaixo do preço atual
//   queda de 0,01% a 10,00% → mantém o preço atual (piso no custo em qualquer caso)
//   (Tiago, 24/09/2026: era 20%; passou pra 10%)
const QUEDA_MIN = 0.10;
...
function calcularItem(e, params) {
  ...
  const AJ = parseAjuste(params?.ajuste_centavos), A = v => aplicarAjuste(v, AJ);
  ...
  const variacao = ca != null && ci > 0 ? +(ci / ca - 1).toFixed(6) : null;      // 6 casas: variação sem arredondar na tela
  const margemSeMantem = pa != null && ci > 0 && pa > 0 ? +((pa - ci) / pa).toFixed(6) : null;   // margem sobre a venda (mesma base da margem de cadastro)
  ...
  const antes = sug; sug = A(sug); out.ajuste = sug !== antes ? { de: antes, para: sug } : null;
  if (sug !== antes && pa != null) status = statusPor(sug);
...
module.exports = { arred, cents, calcularItem, calcularRegistro, resumo, TOL_SEM_MUDANCA, POLITICAS, TERMINACOES, QUEDA_MIN, DESCONTO_PRECO, AJUSTE_CENTAVOS_PADRAO, parseAjuste, aplicarAjuste, validaAjuste };
